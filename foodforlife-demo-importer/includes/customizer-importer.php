 ...
 */
class FoodForLife_Demo_Customizer_Importer {
	...
	public function import( $file ) {
		...
		// S-DEMO: Use unserialize with allowed_classes=false to prevent PHP Object Injection.
		// The customizer export format requires PHP serialization, but we block object instantiation.
		$data = @unserialize( $raw, array( 'allowed_classes' => false ) );

		// S-DEMO: Strict validation — data must be an array with required keys.
		if ( ! is_array( $data ) || ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) {
			return new WP_Error(
				'customizer_import_data_error',
				esc_html__( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'foodforlife' )
			);
		}

		// S-DEMO: Validate mods is an array.
		if ( ! is_array( $data['mods'] ) ) {
			return new WP_Error(
				'customizer_import_data_error',
				esc_html__( 'Invalid customizer mods format.', 'foodforlife' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'foodforlife' )
			);
		}
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new FoodForLife_Demo_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
