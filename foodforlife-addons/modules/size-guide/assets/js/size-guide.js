jQuery(document).ready(function ($) {
	...
	// Render tabs.
	var $tabs = $( '#foodforlife-size-guide-tabs' ),
		$tabsNav = $( '.foodforlife-size-guide-tabs--tabs', $tabs ),
		$addTab = $( '.add-new-tab', $tabsNav ),
		templates = {
			tab: wp.template( 'foodforlife-size-guide-tab' ),
			panel: wp.template( 'foodforlife-size-guide-panel' )
		};

	if ( ! _.isUndefined( foodforlifeSizeGuideTables ) ) {
		for ( var i = 0; i < foodforlifeSizeGuideTables.tables.length; i++ ) {
			$addTab.before( templates.tab( {
				index: i,
				tab: foodforlifeSizeGuideTables.tabs[i]
			} ) );

			$tabs.append( templates.panel( {
				index: i,
				name: foodforlifeSizeGuideTables.names[i],
				description: foodforlifeSizeGuideTables.descriptions[i],
				table: foodforlifeSizeGuideTables.tables[i],
				information: foodforlifeSizeGuideTables.information[i]
			} ) );

			$tabs.find( '.foodforlife-size-guide-table-editor[data-tab='+ i +']' ).find( '.foodforlife-size-guide-table' ).foodforlifeEditTable();

			$addTab.data( 'index', i );

			if ( 0 === i ) {
				$tabsNav.children( ':eq(0)' ).addClass( 'active' );
				$tabs.find( '.foodforlife-size-guide-table-editor[data-tab='+ i +']' ).addClass( 'active' );
			}
	...
	// Change active tab.
	$tabsNav.on( 'click', '.foodforlife-size-guide-table-tabs--tab', function() {
		var $el = $( this );
		...
		$tabs.find( '.foodforlife-size-guide-table-editor[data-tab='+ $el.data( 'tab' ) +']' ).addClass( 'active' ).siblings( '.active' ).removeClass( 'active' );
	} );

	// Edit tab title.
	$tabsNav
		.on( 'click', '.edit-button', function() {
			$( this ).closest( '.foodforlife-size-guide-table-tabs--tab' ).addClass( 'editting' ).children( 'input' ).removeClass( 'hidden' );
		} )
		.on( 'click', '.confirm-button', function() {
			$( this ).closest( '.foodforlife-size-guide-table-tabs--tab' ).removeClass( 'editting' ).children( 'input' ).addClass( 'hidden' );
		} )
		.on( 'input', 'input', function() {
			$( this ).siblings( '.foodforlife-size-guide-table-tabs--tab-text' ).text( this.value );
		} );
	...
	$tabsNav.on( 'click', '.add-new-tab', function() {
		...
		$tabs.find( '.foodforlife-size-guide-table-editor[data-tab='+ index +']' ).find( '.foodforlife-size-guide-table' ).foodforlifeEditTable();
	...
	$tabs.on( 'click', '.delete-table', function( event ) {
		...
		var $deletedPanel = $( this ).closest( '.foodforlife-size-guide-table-editor' ),
	...
	$tabs.on( 'tab_added tab_removed', function() {
		if ( $tabs.children( '.foodforlife-size-guide-table-editor' ).length <= 1 ) {
...
(function ($, window, i) {
	...
	$.fn.foodforlifeEditTable = function (options) {

		// Settings
		var s = $.extend({
				data: [ ['']],
				tableClass: 'foodforlife-table-edit',
				jsonData: false,
				headerCols: false,
				maxRows: 999,
				first_row: true,
