;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('foodforlife_ext/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateFoodForLifeExtView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-foodforlife-ext',
		ui: function ui() {
		...
		onPreviewButtonClick: function onPreviewButtonClick() {
			$e.route('foodforlife_ext/preview', {
				model: this.model
			});
		},
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			foodforlife_ext_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && foodforlife_ext_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-foodforlife-ext',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'foodforlife_ext' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-foodforlife-ext',
		...
		getChildView: function getChildView(childModel) {
			return TemplateLibraryTemplateFoodForLifeExtView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = foodforlife_ext_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = foodforlife_ext_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : foodforlife_ext_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(foodforlife_ext_templates_lib.templates.getFilter('text') || foodforlife_ext_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('foodforlife_ext' === foodforlife_ext_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('foodforlife_ext' === foodforlife_ext_templates_lib.templates.getFilter('source') && 'page' !== foodforlife_ext_templates_lib.templates.getFilter('type') && 'homepage' !== foodforlife_ext_templates_lib.templates.getFilter('type') && 'woocommerce' !== foodforlife_ext_templates_lib.templates.getFilter('type') ) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			foodforlife_ext_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
			foodforlife_ext_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			foodforlife_ext_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo-foodforlife-ext',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-foodforlife-ext',
		...
		onSyncClick: function() {
			...
			foodforlife_ext_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('foodforlife_ext');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('foodforlife_ext').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getLogoOptions: function() {
			return {
				title: 'FoodForLife Library',
				click: function click() {
					$e.run('foodforlife_ext/open', {
						toDefault: true
					});
					return false;
				}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.onDocumentLoaded = function( document ) {
			this.setDefaultRoute( 'templates/homepages' );
		...
		Component.prototype.activateTab = function(tab) {
			$e.routes.saveState('foodforlife_ext');
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'foodforlife_ext' ) ) {
		...
		Component.prototype.maybeOpenLibrary = function() {
			if ('#foodforlife_ext' === location.hash) {
				$e.run('foodforlife_ext/open');
				location.hash = '';
			}
		};
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('foodforlife_ext_get_library_data', ajaxOptions);
		};
		...
		this.filterTemplates = function () {
			return templatesCollection.filter(function (model) {
				if ('foodforlife_ext' !== model.get('source')) {
					return false;
				}

				return 'block' === model.get('type') || 'page' === model.get('type') || 'homepage' === model.get('type') || 'woocommerce' === model.get('type');
			});
		};
	...
	foodforlife_ext_templates_lib.templates = new TemplateLibraryManager();
	foodforlife_ext_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.elementor-add-section-area-button.foodforlife-ext-add-template-button { padding: 12px;align-items: center; }';
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button foodforlife-ext-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add FoodForLife Template' );
		$templateLibBtn.html( '<img src="' + foodforlife_ext_templates_lib.logoUrl + '" />' );
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.foodforlife-ext-add-template-button' ).remove();
