...
class WCFM {
	...
	public function __construct() {
		...
		add_action('foodforlife_woocommerce_product_quickview_summary', array( $this, 'vendor_name' ), 30 );
		add_action('foodforlife_woocommerce_product_quickadd_summary_header', array( $this, 'vendor_name' ), 30 );

		// Store list sidebar panel
		add_action( 'foodforlife_after_close_site_footer', array( $this, 'store_list_button_sidebar' ), 100 );
		add_action( 'foodforlife_after_close_site_footer', array( $this, 'store_list_sidebar_panel' ), 100 );

		// Store page sidebar panel
		add_action( 'foodforlife_after_close_site_footer', array( $this, 'store_page_button_sidebar' ), 100 );
		add_action( 'foodforlife_after_close_site_footer', array( $this, 'store_page_sidebar_panel' ), 100 );
	}
	...
	public function body_classes( $classes ) {
		if ( ( function_exists( 'wcfmmp_is_stores_list_page' ) && wcfmmp_is_stores_list_page() )
			...
			&& intval( \FoodForLife\Helper::get_option( 'vendor_store_style_theme' ) )) {
				$classes[] = 'foodforlife-store-style-theme';
			}

		if ( function_exists( 'wcfmmp_is_stores_list_page' ) && wcfmmp_is_stores_list_page() ) {
			$classes[] = 'foodforlife-wcfm-store-list';
		}

		if ( function_exists( 'wcfm_is_store_page' ) && wcfm_is_store_page() ) {
			$classes[] = 'foodforlife-wcfm-store-page';
		}
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'foodforlife-wcfm', get_template_directory_uri() . '/assets/css/vendors/wcfm.css', array(), '20241003' );

		wp_enqueue_script( 'foodforlife-wcfm', get_template_directory_uri() . '/assets/js/vendors/wcfm.js', array(), '20240508', array('strategy' => 'defer') );
	}
	...
	public function product_card_layout() {
		if( \FoodForLife\Helper::get_option( 'product_card_vendor_name' ) == 'none' ) {
			return;
		}

		if ( \FoodForLife\Helper::get_option( 'product_card_vendor_position' ) == 'after-thumbnail' ) {
	...
	public function vendor_name() {
		...
		$vendor_type = \FoodForLife\Helper::get_option( 'product_card_vendor_name' );
		...
		$classes .= ' vendor-position-' . \FoodForLife\Helper::get_option( 'product_card_vendor_position' );
		$classes .= $product->get_attributes() ? ' show-attributes' : '';
		$vendor_avatar = $vendor_type == 'avatar' ? $store_logo : '<span class="vendor-name-text">' . esc_html__( 'By', 'foodforlife' ) . '</span>';
		?>
	...
	function sold_by_single_product_open() {
		echo '<div class="foodforlife-sold-by-template">';
	}
	...
	public function button_sidebar() {
		?>
			<div class="wcfm-sidebar-panel__button position-fixed d-flex align-items-center justify-content-center" data-toggle="off-canvas" data-target="wcfm-sidebar-panel">
				<?php echo \FoodForLife\Icon::get_svg( 'sidebar' ); ?>
			</div>
		<?php
	}
			...
			<div id="wcfm-sidebar-panel" class="wcfm-sidebar-panel offscreen-panel--side-right">
				...
				<div class="sidebar__container">
					<div class="sidebar__header">
						<?php echo esc_html__( 'Sidebar', 'foodforlife' ); ?>
					</div>
