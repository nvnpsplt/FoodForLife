...
class Topbar {
	...
	public static function items( $items ) {
		...
		foreach ( $items as $item ) {
			$item['item'] = $item['item'] ? $item['item'] : key( \FoodForLife\Options::topbar_items_option() );
			...
			switch ( $item['item'] ) {
				...
				case 'menu':
					if ( ! empty( \FoodForLife\Helper::get_option( 'topbar_menu' ) ) ) {
						wp_nav_menu( array(
				...
				case 'custom-html':
					$topbar_custom_html = \FoodForLife\Helper::get_option('topbar_custom_html');
				...
				default:
					do_action( 'foodforlife_header_topbar_item', $item['item'] );
					break;
			}
		}
	}
	...
	public static function slides() {
		$slides = (array) \FoodForLife\Helper::get_option( 'topbar_slides' );

		$slides = apply_filters('foodforlife_topbar_slides', $slides);
