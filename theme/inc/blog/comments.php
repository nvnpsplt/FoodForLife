...
class Comments {
	...
	public function comment_form_fields($fields) {
		...
		$fields_new = array(
			'author' => '<p class="comment-form-author">' .
			            '<input id ="author" placeholder="' . esc_attr__( 'Your Name*', 'foodforlife' ) . ' " name="author" type="text" required value="' . esc_attr( $comment_author ) .
			            '" size="30" maxlength="245" autocomplete="name"' . $aria_req . ' /></p>',

			'email' => '<p class="comment-form-email">' .
			           '<input id ="email" placeholder="' . esc_attr__( 'Your Email*', 'foodforlife' ) . '" name="email" type="email" required value="' . esc_attr( $comment_author_email ) .
			           '" size="30" maxlength="100" aria-describedby="email-notes" autocomplete="email"' . $aria_req . ' /></p>',

			'url' => '<p class="comment-form-url">' .
			         '<input id ="url" placeholder="' . esc_attr__( 'Your Website (Optional)', 'foodforlife' ) . '" name="url" type="text" value="' . esc_attr( $comment_author_url ) .
	 ...
	 * @return string
	 */
	public static function foodforlife_comment( $comment, $args, $depth ) {
	...
	public static function comment_template( $comments ) {
		...
		$date      = sprintf( esc_html__( '%1$s at %2$s', 'foodforlife' ), $comments['comment_date'], $comments['comment_time'] );
		$output[]  = sprintf( '<a href="%s" class="date fs-14 text-base">%s</a>', esc_url( $comments['comment_link'] ), $date );
		$output[]  = '</div>';
		if ( $comments['comment_approved'] == '0' ) {
			$output[] = sprintf( '<p class="comment-text mb-15"><em class="comment-awaiting-moderation fs-14">%s</em></p>', esc_html__( 'Your comment is awaiting moderation.', 'foodforlife' ) );
		} else {
		...
		if ( current_user_can( 'edit_comment', $comments['comment_ID'] ) ) {
			$output[] = sprintf( '<a class="comment-edit-link" href="%s">%s</a>', esc_url( admin_url( 'comment.php?action=editcomment&amp;c=' ) . $comments['comment_ID'] ), esc_html__( 'Edit', 'foodforlife' ) );
		}
