...
class Page_Header {
	...
	public function __construct() {
		...
		add_action( 'foodforlife_after_header', array( $this, 'show_page_header' ), 99 );
	}
	...
	public static function get_items() {
		...
		return apply_filters( 'foodforlife_get_page_header_elements', self::$items );
	}
	...
	public static function classes( $classes ) {
		...
		echo apply_filters('foodforlife_page_header_classes', $classes);
	}
	...
	public static function title() {
		...
		return apply_filters('foodforlife_page_header_title', $title);
	}
	...
	public static function description() {
		if( in_array( 'description', self::get_items() ) ) {
			...
			$number_lines = \FoodForLife\Helper::get_option( 'page_header_description_lines' );
			$number_lines = apply_filters('foodforlife_page_header_description_lines', $number_lines);
			$style = $number_lines ? 'style="--ffl-page-header-description-lines: '. esc_attr( $number_lines ) .'"' : '';
			if( $description ) {
	...
	public function get_archive_title( $title ) {
		if ( is_search() ) {
			...
			if( (int) $wp_query->found_posts > 0 ) {
				$title = sprintf(
					esc_html__( '%d Search Results for: "%s"', 'foodforlife' ),
					(int) $wp_query->found_posts,
					esc_html( get_search_query() )
				);
			} else {
				$title = sprintf( esc_html__( 'Search Results', 'foodforlife' ) );
			}
		} elseif ( is_404() ) {
			$title = sprintf( esc_html__( 'Page Not Found', 'foodforlife' ) );
		} elseif ( is_page() ) {
			$title = get_the_title(\FoodForLife\Helper::get_post_ID());
		} elseif ( is_home() && is_front_page() ) {
			$title = esc_html__( 'The Latest Posts', 'foodforlife' );
		} elseif ( is_home() && ! is_front_page() ) {
