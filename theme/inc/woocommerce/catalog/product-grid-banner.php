...
class Product_Grid_Banner {
	...
	public function __construct() {
		if ( Helper::get_option( 'product_grid_banner' ) ) {
			add_filter( 'foodforlife_shop_loop_has_banner', '__return_true' );
			add_filter( 'woocommerce_shop_loop', array( $this, 'product_grid_banner' ), 15 );
			add_action( 'foodforlife_shop_loop_elementor', [ $this, 'product_grid_banner_html' ], 15 );
			add_filter( 'foodforlife_shop_loop_banner_position', [ $this, 'get_product_grid_banner_position' ] );
		}
	...
	public function product_grid_banner() {
		...
		$this->product_grid_banner_html();
	}

	public function product_grid_banner_html() {
		if( ! is_shop() && ! is_product_category() ) {
			return;
		}
		...
		$aria_label = esc_html__( 'Link for', 'foodforlife' ) . ' ' . esc_html__( 'Product Grid Banner', 'foodforlife' );
		echo '<li class="product ffl-product-grid-banner">';
		echo '<a class="ffl-ratio--product-image ffl-lazy-load" href="'. esc_url( $link ) .'" aria-label="'. esc_attr( $aria_label ) .'">';
			echo '<img width="100%" height="100%" src="'. esc_url( $image ) .'" alt="'. esc_attr__( 'Product Grid Banner', 'foodforlife' ) .'">';
			echo '<div class="ffl-lazy-load-image"><span class="ffl-lazy-load-image__loader"></span></div>';
		echo '</a>';
		echo '</li>';
	}
