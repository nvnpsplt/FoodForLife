...
class Sidebar {
	...
	public function __construct() {
		// Sidebar
		add_filter( 'foodforlife_primary_sidebar_id', array( $this, 'sidebar_id' ), 10 );
	...
	public function catalog_sidebar_before_content( $index ) {
		...
		if ( ! apply_filters( 'foodforlife_get_catalog_sidebar_before_content', true ) ) {
			return;
		}

		$classes = 'sidebar__button-close ffl-button ffl-button-icon ffl-button-text position-absolute top-13 end-20 z-1';
		$classes .= Helper::get_option( 'product_filter_type' ) !== 'popup' ? ' d-none-xl' : '';

		?>
		<div class="sidebar__backdrop"></div>
        <div class="sidebar__container">
			<?php echo \FoodForLife\Icon::get_svg( 'close', 'ui', array( 'class' => $classes ) ); ?>
