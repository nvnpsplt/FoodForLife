...
class Top_Categories {
	...
	public function __construct() {
		add_action( 'foodforlife_after_site_content_open', array( $this, 'top_categories' ), 10 );
	}
	...
	public function top_categories() {
		...
		$top_html = '<div class="catalog-top-categories ffl-arrows-middle"></div>';
		...
		foreach( $terms as $term ) {
			...
			$aria_label = esc_html__( 'Link for', 'foodforlife' ) . ' ' . $term->name;

			$ouput[] = sprintf(
						'<div class="catalog-top-categories__item overflow-hidden swiper-slide %s">
							<a class="catalog-top-categories__inner ffl-arrows-middle__image ffl-ratio ffl-hover-zoom ffl-hover-effect ffl-image-rounded overflow-hidden position-relative" href="%s" aria-label="%s">
								%s
							</a>
		...
		echo sprintf(
				'<div class="catalog-top-categories position-relative w-100 mb-40 mb-md-60 foodforlife-swiper swiper foodforlife-carousel--elementor navigation-class-arrows navigation-class--tabletdots navigation-class--mobiledots ffl-arrows-middle" data-swiper="%s" data-desktop="%s" data-tablet="%s" data-mobile="%s">
				...
				$mobile_columns,
				implode( '', $ouput ),
				\FoodForLife\Icon::get_svg( 'left-mini', 'ui', 'class=ffl-button-light foodforlife-swiper-button swiper-button swiper-button-prev' ),
				\FoodForLife\Icon::get_svg( 'right-mini', 'ui', 'class=ffl-button-light foodforlife-swiper-button swiper-button swiper-button-next' )
			);
	}
