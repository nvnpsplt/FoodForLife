...
class Language {
	...
	public static function language_switcher( $display = 'list' ) {
		...
		\FoodForLife\Theme::set_prop( 'popovers', 'language' );

		if( $display == 'list' ) {
			foreach ( (array) $languages as $code => $language ) {
				if ( ! $language['active'] ) {
					$lang_list[] = sprintf(
						'<li class="foodforlife-language__menu-item py-4 %s"><a class="no-underline text-base text-hover-color" href="%s">%s</a></li>',
						esc_attr( $code ),
					...
					array_unshift( $lang_list, sprintf(
						'<li class="foodforlife-language__menu-item py-4 %s"><a class="no-underline text-dark text-hover-color active" href="%s">%s</a></li>',
						esc_attr( $code ),
						esc_url( $language['url'] ),
						esc_html( $language['native_name'] )
					) );
			...
			?>
			<label><?php esc_html_e( 'Language', 'foodforlife' ); ?></label>
			<select name="language" id="foodforlife_language" class="language_select preferences_select">
				<?php
