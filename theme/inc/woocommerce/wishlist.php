...
class Wishlist {
	...
	public function wishlist_svg_icon($svg, $icon) {
		if( $icon == 'heart' ) {
			$svg = \FoodForLife\Icon::inline_svg('icon=icon-wishlist');
		} elseif( $icon == 'heart-filled' ) {
			$svg = \FoodForLife\Icon::inline_svg('icon=icon-wishlist');
		}

		if ( ! empty( \FoodForLife\Helper::get_option( 'wishlist_icon_svg' ) ) ) {
			$svg = '<span class="foodforlife-svg-icon foodforlife-svg-icon--custom-wishlist">' . \FoodForLife\Icon::sanitize_svg( \FoodForLife\Helper::get_option( 'wishlist_icon_svg' ) ) . '</span>';

			if ( ! empty( \FoodForLife\Helper::get_option( 'wishlist_icon_svg_filled' ) ) && str_contains( $icon, '-filled' ) ) {
				$svg = '<span class="foodforlife-svg-icon foodforlife-svg-icon--custom-wishlist">' . \FoodForLife\Icon::sanitize_svg( \FoodForLife\Helper::get_option( 'wishlist_icon_svg_filled' ) ) . '</span>';
			}
		} 
	...
	public function wishlist_button_product_loop( $html, $args ) {
		...
		return sprintf(
			'<a href="%s" class="ffl-button-icon ffl-button-light ffl-tooltip %s" %s data-product_title="%s" data-tooltip="%s" data-tooltip_added="%s" data-tooltip_position="%s">' .
			...
			apply_filters( 'foodforlife_wishlist_loop_tooltip_position', 'left' ),
	...
	public function wishlist_empty_message() {
		return '<p class="mb-0 text-center text-base fw-normal">' . esc_html__( 'No products were added to the shop page.', 'foodforlife' ) . '</p>';
	}
	...
	public function wishlist_return_button() {
		return '<a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="button wc-backward foodforlife-button ffl-button mt-20 py-17 px-30 ffl-button-hover-effect min-w-200">' . esc_html__( 'Back to Shopping', 'foodforlife' ) . '</a>';
	}
