...
$post_type = 'product';
if( \FoodForLife\Helper::is_blog() || is_singular('post') ) {
	$post_type = 'post';
}
		...
		<div class="modal__wrapper">
			<div class="modal__header">
				<div class="container-xxl">
					<h4 class="search-modal__title <?php echo esc_attr( $modal_title_class ); ?>"><?php esc_html_e('Search Our Site', 'foodforlife'); ?></h4>
					<div class="d-flex align-items-center gap-0 <?php echo esc_attr( $modal_sidebar_class ); ?>">
						<form class="search-modal__form ffl-instant-search__form flex-grow-1 position-relative" method="get" action="<?php echo esc_url( home_url( '/' ) ) ?>">
							<input type="text" name="s" class="search-modal__field ffl-instant-search__field w-100" placeholder="<?php esc_attr_e("I'm looking for…", 'foodforlife') ?>" autocomplete="off">
							<input type="hidden" name="post_type" class="search-modal__post-type" value="<?php echo esc_attr( $post_type ); ?>">
							<a href="#" class="close-search-results position-absolute end-45 top-0 ffl-button ffl-button-icon invisible"><?php echo \FoodForLife\Icon::get_svg( 'close', 'ui'); ?></a>
							<button type="submit" class="search-modal__button ffl-instant-search__button ffl-button ffl-button-icon position-absolute end-5 top-0">
			...
			<div class="modal__content woocommerce">
				<div class="container-xxl">
					<?php do_action('foodforlife_search_modal_before_form'); ?>
					<div class="modal__content-suggestion d-flex flex-column gap-25 mt-25">
						<?php
						\FoodForLife\Header\Search::get_trending();
						\FoodForLife\Header\Search::get_products();
						?>
					</div>
					<?php do_action('foodforlife_search_modal_after_form'); ?>
				</div>
			</div>
		</div>
	</div>
