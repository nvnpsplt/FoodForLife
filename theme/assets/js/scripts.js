(function ($) {
	...
	foodforlife.init = function () {
		...
		foodforlife.$body.on('foodforlife_products_filter_request_success foodforlife_get_products_ajax_loaded', function () {
			foodforlife.productSaleMarquee();
        });
		...
		foodforlife.$body.on( 'foodforlife_single_product_variation_form_init', function(){
			foodforlife.productVariation();
		} );
	 ...
	 */
	foodforlife.toggleOffCanvas = function() {
		$( document.body ).on( 'click', '[data-toggle="off-canvas"]', function( event ) {
			var target = '#' + $( this ).data( 'target' );

			if ( $( target ).hasClass( 'offscreen-panel--open' ) ) {
				foodforlife.closeOffCanvas( target );
			} else if ( foodforlife.openOffCanvas( target ) ) {
			...
			foodforlife.closeOffCanvas( this );
		} ).on( 'keyup', function ( e ) {
			if ( e.keyCode === 27 ) {
				foodforlife.closeOffCanvas();
			}
		} );
	};
	 ...
	 */
	foodforlife.openOffCanvas = function( target ) {
		var $target = $( target );
		...
		if( $('body').hasClass('foodforlife-header-sidebar-categories-enable')) {
		...
		$( document.body ).addClass( 'offcanvas-opened ' + $dataId + '-opened' ).trigger( 'foodforlife_off_canvas_opened', [$target] );
	 ...
	 * @param DOM target
	 */
	foodforlife.closeOffCanvas = function( target ) {
		if ( !target ) {
			$( '.offscreen-panel' ).each( function() {
				...
				if( $panel.find('.foodforlife-mini-products-recommended').length ) {
					$panel.find('.foodforlife-mini-products-recommended').removeClass('mini--open');
					setTimeout(function() {
			...
			var $dataId = target.attr( 'id' ) ? target.attr( 'id' ) : target.data( 'id' );
			if( target.find('.foodforlife-mini-products-recommended').length && target.find('.foodforlife-mini-products-recommended').hasClass('mini--open') ) {
				target.find('.foodforlife-mini-products-recommended').removeClass('mini--open');
				setTimeout(function() {
		...
		const navHeight = body.style.getPropertyValue('--ffl-navigation-bar-height');

		$(document.body).removeAttr('style');

		if (navHeight) {
			body.style.setProperty('--ffl-navigation-bar-height', navHeight);
		}

		$( document.body ).removeClass( 'offcanvas-opened' ).trigger( 'foodforlife_off_canvas_closed', [target] );
	}

	/**
	 * Toggle modals.
	 */
	foodforlife.toggleModals = function() {
		$( document.body ).on( 'click', '[data-toggle="modal"]', function( event ) {
			...
			if ( $( target ).hasClass( 'modal--open' ) ) {
				if( $(this).is( 'form.ffl-instant-search__form' ) ) {
					return;
				}

				if( $(this).closest('body').hasClass('offcanvas-opened') ) {
					foodforlife.closeModal( target, false );
				} else {
					foodforlife.closeModal( target );
				}
			} else if ( foodforlife.openModal( target ) ) {
				event.preventDefault();
			}
			...
			if( $(this).closest('body').hasClass('offcanvas-opened') ) {
				foodforlife.closeModal( this, false );
			} else {
				foodforlife.closeModal( this );
			}
		} ).on( 'keyup', function ( e ) {
			if ( e.keyCode === 27 ) {
				foodforlife.closeModal();
			}
		} );
	};
	 ...
	 * @param string target
	 */
	foodforlife.openModal = function( target ) {
		var $target = $( target );
		...
		var $pageWidth = $('body').hasClass('foodforlife-header-sidebar-categories-enable') ? $('#page').width() + parseFloat($('body').css('padding-inline-start')) : $('#page').width();
		var widthScrollBar = window.innerWidth - parseFloat($pageWidth);

		if ( ( $("html").attr("dir") === "rtl" || $("body").css("direction") === "rtl" ) && $('body').hasClass('foodforlife-header-sidebar-categories-enable')) {
		...
		$( document.body ).addClass( 'modal-opened ' + $target.attr( 'id' ) + '-opened' ).trigger( 'foodforlife_modal_opened', [$target] );
	 ...
	 * @param string target
	 */
	foodforlife.closeModal = function( target, removePadding = true, speed = 400 ) {
		if ( !target ) {
		...
		if( ! $(target).parents('.modal').length ) {
			...
			$( document.body ).removeClass( 'modal-opened' ).trigger( 'foodforlife_modal_closed', [target] );
		}
	 ...
	 * @param string target
	 */
	foodforlife.openPopover = function( target ) {
		var $target = $( target );
		...
		$( document.body ).addClass( 'popover-opened ' + $target.attr( 'id' ) + '-opened' ).trigger( 'foodforlife_popover_opened', [$target] );
	 ...
	 * @param string target
	 */
	foodforlife.closePopover = function( target, removePadding = true ) {
		if ( !target ) {
		...
		$( document.body ).removeClass( 'popover-opened' ).trigger( 'foodforlife_popover_closed', [target] );
	}

	/**
     * Progressbar
     */
    foodforlife.progressBar = function () {
		let progressContainer = $("#ffl-progress-container");
        let progressBar = $("#ffl-progress-bar");
		...
		$(document.body).on('foodforlife_progress_bar_start', function() {
			startProgress();
		});

		$(document.body).on('foodforlife_progress_bar_update', function(event, percent) {
			updateProgress(percent);
		});

		$(document.body).on('foodforlife_progress_bar_complete', function() {
			completeProgress();
		});
	 ...
	 */
	foodforlife.lazyLoadVideo = function() {
		...
		foodforlife.$body.on('foodforlife_get_products_ajax_loaded foodforlife_products_filter_request_success', function() {
			convertVideoSrcToDataSrc();
			lazyLoadVideo();
		});
		...
		function convertVideoSrcToDataSrc() {
			...
			var regularVideos = document.querySelectorAll('video[src]:not([data-src]):not(.ffl-lazy-video-processed)');
			...
			regularVideos.forEach(function(video) {
				...
				video.classList.add('ffl-lazy-video');

				// Mark as processed to avoid double-processing
				video.classList.add('ffl-lazy-video-processed');
			});
		}

		function lazyLoadVideo() {
			// Look for videos with data-src attribute or with the ffl-lazy-video class
			var videos = document.querySelectorAll('video[data-src], video.ffl-lazy-video');
			...
			var observer = new IntersectionObserver(function(entries, observer) {
				entries.forEach(function(entry) {
					if (entry.isIntersecting) {
						...
						// Add a loading class
						video.classList.add('ffl-video-loading');
						...
						video.addEventListener('loadeddata', function onVideoLoaded() {
							...
							video.classList.remove('ffl-video-loading');
							video.classList.add('ffl-video-loaded');
							...
							foodforlife.$body.trigger('foodforlife_lazy_load_video_loaded', [video]);
	...
	foodforlife.foodforlifeSwiper = function() {
		var $els = $('.foodforlife-swiper');
		...
		$els.each(function() {
			...
			if( $element.hasClass( 'foodforlife-product-carousel') ) {
		...
		function updateArrowPosition() {
			if ( ! $els.hasClass('ffl-arrows-middle') ) {
				return;
			}

			$els.find(".swiper-button-next, .swiper-button-prev").each(function () {
				let slide = $(this).closest(".ffl-arrows-middle").find(".ffl-arrows-middle__image, .product-thumbnail .woocommerce-loop-product__link img");
				if (slide.length) {
					let imageHeight = slide.height();
					let topPosition = imageHeight / 2;

					$(this).get(0).style.setProperty('--ffl-arrow-top', `${topPosition}px`);
				}
			});
		}
	};
	 ...
	 */
	foodforlife.loadMorePosts = function() {
		// Infinite scroll.
		if ( $( '.foodforlife-pagination--blog' ).hasClass( 'foodforlife-pagination--infinite' ) ) {
			var waiting = false,
				endScrollHandle;
		...
		function infiniteScoll() {
			var $navigation = $( '.foodforlife-pagination--blog.foodforlife-pagination--ajax' ),
				$button = $( 'a', $navigation );

			if ( foodforlifeIsVisible( $button ) && $button.length && ! $button.hasClass( 'loading' ) ) {
		...
		// Load More
		$( document.body ).on( 'click', '.foodforlife-pagination--blog.foodforlife-pagination--loadmore a', function( event ) {
			event.preventDefault();
		...
		function loadPosts( $button ) {
			var $posts = $button.closest('#primary').find('#main'),
				...
				$navigation = $button.closest( '.foodforlife-pagination' ),
			...
			$.get( url, function( response ) {
				...
				$posts.addClass( 'ffl-fadeinup ffl-animated' );

				let delay = 0.5;
				$posts.each( function( index, post ) {
					$( post ).css( '--ffl-fadeinup-delay', delay + 's' );
					delay = delay + 0.1;
				} );
				...
				if( $posts.hasClass( 'ffl-animated' ) ) {
					setTimeout( function() {
						$posts.removeClass( 'ffl-animated' );
					}, 10 );
				}
				...
				foodforlife.postFound();
				$(document.body).trigger('foodforlife_progress_bar_complete');

				if ( foodforlifeData.blog_nav_ajax_url_change ) {
	...
	foodforlife.postFound = function (el) {
	...
	// Lazy Load Image
	foodforlife.lazyLoadImage = function () {
		lazyLoadImage();
		foodforlife.$body.on( 'foodforlife_get_products_ajax_loaded foodforlife_products_filter_request_success added_to_cart wc_fragments_refreshed wc_fragments_loaded', function() {
			lazyLoadImage();
		});

		function lazyLoadImage() {
			var $lazyLoadImage = foodforlife.$body.find('.ffl-lazy-load-image'),
	...
	// Product Attribute
	foodforlife.productAttribute = function () {
		foodforlife.$body.on( 'click', '.product-variation-item', function (e) {
            e.preventDefault();
			if( $(this).hasClass('selected') ) {
				return;
			}

			if( ! $(this).closest('.product-variation-items').hasClass('ffl-variation-hover') ) {
				return;
			}
		...
		foodforlife.$body.on('mouseover', '.product-variation-items', function (e) {
            e.preventDefault();
	...
	foodforlife.instantSearch = function() {
		...
		if( $selector.hasClass( 'search-type-sidebar' ) ) {
			...
			foodforlife.$body.on( 'foodforlife_ajax_search_request_success', function() {
				foodforlife.lazyLoadImage();
		...
		if( $selector.hasClass('search-type-popup') ) {
			if( $selector.hasClass('modal-to-form') ) {
				foodforlife.$body.on('click', function(event) {
					...
					if( $selector.hasClass('modal--open') ) {
						foodforlife.closeModal( $selector );
					}
				});
			}
			...
			foodforlife.$body.on( 'foodforlife_ajax_search_request_success', function() {
				...
				if ( foodforlife.$window.width() > 1199 ) {
					foodforlife.foodforlifeSwiper();
				} else {
					$selector.find( '.swiper' ).addClass( 'destroy-swiper' );
				}
			});
		...
		$( document.body ).on( 'click', '[data-target="search-modal"]', function() {
			if ( foodforlife.$window.width() > 1199 ) {
				if( $selector.hasClass('search-type-popup') && $selector.hasClass('modal-to-form') ) {
					...
					$selector.css('--ffl-modal-top-spacing', top + 'px');
				}
			}
		});

		$(document.body).on( 'foodforlife_modal_opened', function() {
	 ...
	 */
	foodforlife.formFieldFocus = function() {
		...
		.on('click', '.showlogin', function() {
			$(this).closest('.woocommerce').find('.ffl-button-login-mode').trigger('click');
		});

		foodforlife.$window.on("load", function() {
	 ...
	 * Login Popup
	 */
	foodforlife.loginPopup = function() {
		var $modal = $( '#login-modal' );

		$modal
		.on( 'click', '.ffl-button-register-mode', function(e) {
			e.preventDefault();
		...
		.on( 'click', '.ffl-button-login-mode', function(e) {
			e.preventDefault();
	 ...
	 */
	foodforlife.loginModalAuthenticate = function () {
		var $modal = $( '#login-modal' ),
			xhr = null;
		...
		function getLoginAJAX( form, formData, event ) {
			...
			$(document.body).trigger('foodforlife_progress_bar_start');
			xhr = $.post(
				foodforlifeData.admin_ajax_url,
				formData,
				function (response) {
					if ( ! response.success ) {
	 ...
	 */
	foodforlife.registerPopup = function () {
	 ...
	 * Change product quantity
	 */
	foodforlife.productQuantityNumber = function () {
		var debounceTimeout = null;
		foodforlife.$body.on('click', '.foodforlife-qty-button', function (e) {
			e.preventDefault();
	 ...
	 * Quick view modal.
	 */
	foodforlife.productQuickView = function() {
		$( document.body ).on( 'click', '.foodforlife-quickview-button', function( event ) {
			event.preventDefault();
			...
			var $el = $( this ),
				...
				ajax_url = foodforlifeData.ajax_url.toString().replace('%%endpoint%%', 'product_quick_view');
			...
			$.post(
				ajax_url,
				{
					action    : 'foodforlife_get_product_quickview',
					product_id: product_id,
					security  : foodforlifeData.product_quickview_nonce
				},
				function( response ) {
					$container.find( '.product-quickview' ).replaceWith( response.data );

					foodforlifeMore( $( '.product-quickview' ).find( '.short-description__more' ) );
					...
					foodforlife.$body.trigger( 'foodforlife_product_quick_view_loaded' );

					if ( $container.find('.foodforlife-countdown').length > 0) {
						$(document.body).trigger('foodforlife_countdown', [$('.foodforlife-countdown')]);
					}
			...
			function update_quickview() {
				...
				$gallery.on( 'foodforlife_update_product_gallery_on_quickview', function(){
					update_product_gallery();
				});
			...
			function update_product_gallery() {
				...
				if ( $slider.find( '.woocommerce-product-gallery__image' ).length > 1 ) {
					...
					if( foodforlifeData.mobile_single_product_gallery_arrows ) {
						$slider.closest('.woocommerce-product-gallery').addClass('woocommerce-product-gallery--has-arrows-mobile');
					}
					$slider.css({'--ffl-swiper-auto-width-mobile': '64%', '--ffl-swiper-auto-fluid-end-mobile': '10px'});
					$slider.attr('data-mobile', '1');
					$slider.wrapInner('<div class="swiper-wrapper"></div>');
					$slider.find('.swiper-wrapper').after('<span class="foodforlife-svg-icon foodforlife-svg-icon__inline foodforlife-svg-icon--icon-back swiper-button-prev swiper-button"><svg width="24" height="24" aria-hidden="true" role="img" focusable="false"> <use href="#icon-back" xlink:href="#icon-back"></use> </svg></span>');
					$slider.find('.swiper-wrapper').after('<span class="foodforlife-svg-icon foodforlife-svg-icon__inline foodforlife-svg-icon--icon-next swiper-button-next swiper-button"><svg width="24" height="24" aria-hidden="true" role="img" focusable="false"> <use href="#icon-next" xlink:href="#icon-next"></use> </svg></span>');
					...
					var options = {
						...
						on: {
							init: function () {
								...
								foodforlife.$body.trigger( 'foodforlife_product_gallery_quickview_init' );
							},
							slideChangeTransitionEnd: function () {
								foodforlife.$body.trigger( 'foodforlife_product_gallery_quickview_slideChangeTransitionEnd' );
							}
						},
						breakpoints: {
							0: {
			...
			function foodforlifeMore ( $selector ) {
	 ...
	 */
	foodforlife.productSaleMarquee = function () {
		var $selector = $( '.foodforlife-sale-flash-marquee' );
		...
		$selector.each(function () {
			...
			if ($this.hasClass('ffl-marquee-initialized')) {
				return;
			}

			$this.addClass('ffl-marquee-initialized');
			$this.closest('li.product').addClass('foodforlife-sale-flash-marquee--enabled');

			var $inner = $this.find('.foodforlife-marquee__inner'),
				$items = $this.find('.foodforlife-marquee__items');
			...
			$inner.imagesLoaded(function () {
				...
				for (let i = 1; i <= amount; i++) {
					...
					item.css('--ffl-marquee-index', i.toString());
					item.attr('data-index', i);

					item.appendTo($inner);
				}

				const originalId = $inner.find('.foodforlife-marquee--original').data('id');

				$inner.find('.foodforlife-marquee--duplicate').each(function () {
					const $dup = $(this);
	 ...
	 * Open Mini Cart
	 */
	foodforlife.openMiniCartPanel = function () {
		if (typeof foodforlifeData.added_to_cart_notice === 'undefined') {
			return;
		}

		if (foodforlifeData.added_to_cart_notice.added_to_cart_notice_layout !== 'mini') {
			return;
		}
		...
		$(document.body).on('adding_to_cart', function () {
            status = true;
			$('#cart-panel').find('.foodforlife-mini-products-recommended').removeClass('mini--open');
		});
		...
		$(document.body).on( 'added_to_cart wc_fragments_refreshed', function () {
			if( status ) {
				...
				if( $('#quick-view-modal').length && $('#quick-view-modal').hasClass('modal--open') ) {
					foodforlife.closeModal('#quick-view-modal', true, 800)
					$close_modal = true;
				}

				if( $close_modal ) {
					setTimeout(function() {
						foodforlife.openOffCanvas( '#cart-panel' );
					}, 800);
				} else {
					foodforlife.openOffCanvas( '#cart-panel' );
				}

				if( $('#cart-panel').find('.foodforlife-mini-products-recommended').length ) {
					setTimeout(function() {
						if($('#cart-panel').hasClass('offscreen-panel--open')) {
							$('#cart-panel').find('.foodforlife-mini-products-recommended').addClass('mini--open');
						}
					}, 1200);
				}
		...
		$(document.body).on('click', '.products-recommended__button-close', function() {
			$(this).closest('.foodforlife-mini-products-recommended').removeClass('mini--open');
		});

		$( document.body ).on( 'click', '.remove_from_cart_button', function() {
			if( $(this).closest('.widget_shopping_cart_content').find('.foodforlife-mini-products-recommended').length ) {
				$(this).closest('.widget_shopping_cart_content').find('.foodforlife-mini-products-recommended').removeClass('mini--open');
			}
		...
		$( document.body ).on( 'wc_fragments_loaded wc_fragments_refreshed', function() {
			if( $('#cart-panel').find('.foodforlife-mini-products-recommended').length && $remove_from_cart ) {
				setTimeout(function() {
					$('#cart-panel').find('.foodforlife-mini-products-recommended').addClass('mini--open');
	...
	foodforlife.updateCartQtyAuto = function() {
		$( document.body ).on( 'change', 'table.cart .qty', function() {
			if (typeof foodforlifeData.update_cart_page_auto !== undefined && foodforlifeData.update_cart_page_auto == '1') {
				foodforlife.$body.find('button[name="update_cart"]').attr( 'clicked', 'true' ).prop( 'disabled', false ).attr( 'aria-disabled', false );
				foodforlife.$body.find('button[name="update_cart"]').trigger('click');
			}
		} );
	 ...
	 * Product Variation
	 */
	foodforlife.productVariation = function () {
		var $productPrice 	 = $( 'div.product .entry-summary .foodforlife-product-price' ),
			...
			variation_args   = [],
			$stickySelector = $( '#foodforlife-sticky-add-to-cart' );

		// Disable outofstock variation when product only has one attribute
		if( ! foodforlifeData.product_clickable_outofstock_variations ) {
		...
		$('.single-product div.product .entry-summary .variations_form:not(.product-select__variation)').on( 'found_variation', function (event, variation) {
			...
			if( !$( 'body' ).hasClass( 'foodforlife-hide-price' ) ) {
				if( $productPrice.length > 0 ) {
					...
					$productPrice = $(this).closest('.entry-summary').find('.foodforlife-product-price');
					if( $productPrice.length > 0 ) {
						if( $productPrice.find( '.variation-price' ).length > 0 ) {
							$productPrice.find( '.variation-price' ).remove();
						}
			...
			foodforlife.variations_image_update(variation, $form);
			...
			var $countdown = $(this).closest('div.product').find('.entry-summary .ffl-countdown-single-product');
			if( $countdown.length ) {
				var $_countdown = $countdown.find('.foodforlife-countdown');
				if( variation.countdown_expire && $_countdown.attr('data-expire') !== variation.countdown_expire ) {
					...
					$_countdown.foodforlife_countdown();
				} else {
					$(window).get(0).clearCountdownInterval( $_countdown.get(0) );

					$_countdown.find('.foodforlife-countdown').empty();
			...
			$buttonATC = $(event.currentTarget).closest( '.entry-summary' ).find( '.single_add_to_cart_button' );
		...
		$('.single-product div.product .entry-summary .variations_form:not(.product-select__variation)').on( 'reset_data', function ( ) {
			...
			if( $productPrice.length > 0 && $productPrice.find( '.variation-price' ).length > 0 ) {
				...
				$productPrice = $(this).closest('div.product').find('.entry-summary .foodforlife-product-price');
				if( $productPrice.length > 0 && $productPrice.find( '.variation-price' ).length > 0  ) {
					$productPrice.find( '.variation-price' ).remove();
					$productPrice.find( '.price' ).css( 'display', '' );
				}
			...
			var $_countdown = $(this).closest('div.product').find('.entry-summary .ffl-countdown-single-product');
			if( $_countdown.length ) {
				var firstCountdown = $_countdown.find('.foodforlife-countdown').get(0);
			...
			var $form = $(this).closest( '.variations_form' );
			foodforlife.variations_image_reset($form);
		});
	 ...
	 */
	foodforlife.variations_image_update = function( variation, $form ) {
		var $product          = $form.closest( '.product' ),
			...
			$gallery_nav      = $product.find( '.foodforlife-product-gallery-thumbnails' );
		...
		if ( variation && variation.image && variation.image.src && variation.image.src.length > 1 ) {
			...
			if ( galleryHasImage ) {
				...
				foodforlife.set_variation_image( $form, variation );
			}

			if( foodforlife.$window.width() > 768 && $product_gallery.hasClass( 'woocommerce-product-gallery--grid' ) ) {
				if( ! foodforlife.setVariationImageToGalleryGrid($gallery_wrapper, $gallery_nav, $form, variation) ) {
					return false;
				}
			} else if( $gallery_wrapper.hasClass('swiper') ) {
				if( ! foodforlife.setVariationImageToGallerySwiper($gallery_wrapper, $gallery_nav, $form, variation) ) {
					return false;
				}
			}
	...
	foodforlife.setVariationImageToGalleryGrid = function($gallery_wrapper, $gallery_nav, $form, variation) {
		...
		if( foodforlife.$window.scrollTop() > $gallery_wrapper.offset().top ) {
			$('html, body').animate({
	...
	foodforlife.setVariationImageToGallerySwiper = function($gallery_wrapper, $gallery_nav, $form, variation) {
	 ...
	 */
	foodforlife.variations_image_reset = function($form) {
		var $product         = $form.closest( '.product' ),
		    $product_gallery = $product.find( '.woocommerce-product-gallery:not(.woocommerce-product-gallery__video)' ),
		    $gallery_wrapper = $product_gallery.find( '.woocommerce-product-gallery__wrapper' ),
		    $gallery_nav     = $product.find( '.foodforlife-product-gallery-thumbnails' ),
	 ...
	 */
	foodforlife.set_variation_image = function($form, variation) {
		var $product         = $form.closest( '.product' ),
		    $product_gallery = $product.find( '.woocommerce-product-gallery:not(.woocommerce-product-gallery__video)' ),
			$gallery_nav     = $product.find( '.foodforlife-product-gallery-thumbnails' ),
		    $gallery_img     = $gallery_nav.find( '.woocommerce-product-gallery__image:eq(0) img' ),
	...
	foodforlife.updateMiniCartQtyAuto = function() {
		var debounceTimeout = null;
		$( document.body ).on( 'change', '.woocommerce-mini-cart .qty', function() {
			...
			debounceTimeout = setTimeout( function() {
				foodforlife.updateCartAJAX( $this );
			}, 500 );

		} );
	};

	foodforlife.updateMiniCartContent = function() {
		updateMiniCartContentHeight();
		$(window).on('resize', updateMiniCartContentHeight);

		function updateMiniCartContentHeight() {
			var $freeShippingBar = $('#cart-panel .foodforlife-free-shipping-bar'),
			...
			$content.css('--ffl-shopping-cart-content-offset', $freeShippingBarHeight + $panelHeader.outerHeight() + 'px');
		}
	};

	foodforlife.removeItemFromMiniCart = function() {
	...
	foodforlife.applyCoupon = function() {
		$( document.body ).on( 'click', '.discount-popover [name="apply_coupon"]', function(e) {
			...
			$.ajax({
				type: 'POST',
				url: wc_add_to_cart_params.wc_ajax_url.toString().replace( '%%endpoint%%', 'foodforlife_apply_coupon' ),
				data: data,
				success: function(response) {
					...
					if( response.coupon_html ) {
						if( $button.closest('.popover__content').find('.foodforlife-mini-cart-coupons').length ) {
							$button.closest('.popover__content').find('.foodforlife-mini-cart-coupons').html(response.coupon_html);
						} else {
							$button.closest('.popover__content').find('form').before('<div class="foodforlife-mini-cart-coupons mb-15 d-flex flex-column gap-5">' + response.coupon_html + '</div>');
						}

						if ( $('.widget_shopping_cart_content').find('.foodforlife-mini-cart__coupons').length ) {
							$('.widget_shopping_cart_content').find('.foodforlife-mini-cart__coupons').html(response.coupon_html);
						} else {
							$('.widget_shopping_cart_content').find('.woocommerce-mini-cart__total').before('<div class="foodforlife-mini-cart__coupons d-flex flex-column gap-5">' + response.coupon_html + '</div>');
						}
					}
		...
		$(document.body).on( 'click', '.discount-popover .woocommerce-remove-coupon, .foodforlife-mini-cart__coupons .woocommerce-remove-coupon', function(e) {
			e.preventDefault();

			var $button = $(this),
				data = {
					action: 'foodforlife_remove_coupon',
			...
			$.ajax({
				type: 'POST',
				url: wc_add_to_cart_params.wc_ajax_url.toString().replace( '%%endpoint%%', 'foodforlife_remove_coupon' ),
				data: data,
				success: function(response) {
					...
					if ( $button.closest( '.foodforlife-mini-cart-coupons' ).length ) {
						if( $button.closest( '.foodforlife-mini-cart-coupons' ).length > 0 ) {
							$button.closest('.cart-discount').remove();
						} else {
							$button.closest( '.foodforlife-mini-cart-coupons' ).remove();
						}
					}

					if ( $button.closest( '.foodforlife-mini-cart__coupons' ).length ) {
						if( match ) {
							if ( $('.discount-popover').find('.foodforlife-mini-cart-coupons').length > 0 ) {
								$('.discount-popover').find('.cart-discount.' + match[0]).remove();
							} else {
								$('.discount-popover').find('.foodforlife-mini-cart-coupons').remove();
							}
						}
					}
		...
		$(document.body).on( 'foodforlife_off_canvas_closed', function(e) {
	 ...
	 */
	foodforlife.updateShippingAddress = function() {
		$( document.body ).on( 'click', '.estimate-popover [name="calc_shipping"]', function(e) {
			...
			data.push({
				name: 'action',
				value: 'foodforlife_update_shipping_address'
			});
			...
			$.ajax({
				type: 'POST',
				url: wc_add_to_cart_params.wc_ajax_url.toString().replace( '%%endpoint%%', 'foodforlife_update_shipping_address' ),
				data: data,
				success: function(response) {
					$notices.html(response.notices);
					$button.removeClass('loading');
	 ...
	 * Quick Edit modal.
	 */
	foodforlife.productQuickEdit = function() {
		$( document.body ).on( 'click', '.foodforlife-quickedit-button', function( event ) {
			event.preventDefault();
			...
			var $el = $( this ),
				...
				$container = $target.find( '.woocommerce' ),
				ajax_url = foodforlifeData.ajax_url.toString().replace('%%endpoint%%', 'product_quick_edit');
		...
		$(document.body).on( 'click', '.quick_edit_add_to_cart_button', function( event ) {
			...
			var formData = $button.closest('form').serializeArray(),
				...
				ajax_url = foodforlifeData.ajax_url.toString().replace('%%endpoint%%', 'quick_edit_update_cart_item');
			...
			$.ajax({
				...
				success: function ( response ) {
					...
					$button.removeClass( 'loading' );
					$button.data('requestRunning', false);
					foodforlife.closeModal( '#quick-edit-modal', false );
					$(document.body).trigger('foodforlife_progress_bar_complete');
				}
			});
		} );
	...
	foodforlife.updateCartAJAX = function ($qty) {
		var $row = $qty.closest('.woocommerce-mini-cart-item'),
			...
			ajax_url = foodforlifeData.ajax_url.toString().replace('%%endpoint%%', 'update_cart_item');
	 ...
	 * Recently viewed
	 */
	foodforlife.recentlyViewedProducts = function () {
		if ( foodforlifeData.recently_viewed_enable !== 'yes' ) {
			return;
		}

		var $recently = foodforlife.$body.find( '.recently-viewed-products' ),
			$recently_heading = foodforlife.$body.find( '.recently-viewed-products__title' ),
			$recently_description = foodforlife.$body.find( '.recently-viewed-products__description' ),
			$recently_columns = $recently.data('columns'),
			ajax_url  = foodforlifeData.ajax_url.toString().replace( '%%endpoint%%', 'foodforlife_get_recently_viewed' ),
			xhr       = null;
		...
		foodforlife.$window.on( 'scroll', function () {
			if ( foodforlifeIsVisible( $recently ) && ! xhr ) {
				loadAjaxRecently();
			}
		}).trigger( 'scroll' );
        ...
        function loadAjaxRecently() {
			...
			xhr = $.post(
				ajax_url,
				{
					action: 'foodforlife_get_recently_viewed',
				},
				function (response) {
					if( response.success ) {
						...
						if ( $recently.find( '.no-products' ).length < 1 ) {
							...
							foodforlife.$body.trigger( 'foodforlife_get_products_ajax_loaded', [$recently, false] );
						}
	 ...
	 * Product Affiliate
	 */
	foodforlife.productAffiliate = function () {
	...
	foodforlife.addedToWishlistNotice = function () {
		if ( typeof foodforlifeData.added_to_wishlist_notice === 'undefined' || ! $.fn.notify ) {
			return;
		}

		foodforlife.$body.on('added_to_wishlist', function (e, $el_wrap) {
		...
		function getaddedToWishlistNotice($content) {
			$content += ' ' + foodforlifeData.added_to_wishlist_notice.added_to_wishlist_text;
			$content += '<a href="' + foodforlifeData.added_to_wishlist_notice.wishlist_view_link + '" class="btn-button">' + foodforlifeData.added_to_wishlist_notice.wishlist_view_text + '</a>';

			var $checkIcon = '<span class="foodforlife-svg-icon message-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></span>',
				$closeIcon = '<span class="foodforlife-svg-icon svg-active"><svg class="svg-icon" aria-hidden="true" role="img" focusable="false" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 1L1 14M1 1L14 14" stroke="#A0A0A0" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"></path></svg></span>';
			...
			$.notify('&nbsp', {
				autoHideDelay: foodforlifeData.added_to_wishlist_notice.wishlist_notice_auto_hide,
				className: 'success',
				style: 'foodforlife',
	...
	foodforlife.addedToCompareNotice = function () {
		if ( typeof foodforlifeData.added_to_compare_notice === 'undefined' || ! $.fn.notify ) {
			return;
		}

		foodforlife.$body.on( 'added_to_compare', function (e, $el_wrap) {
			var content = $el_wrap.data('product_title');
		...
		function getaddedToCompareNotice($content) {
			$content += ' ' + foodforlifeData.added_to_compare_notice.added_to_compare_text;
			$content += '<a href="' + foodforlifeData.added_to_compare_notice.compare_view_link + '" class="btn-button">' + foodforlifeData.added_to_compare_notice.compare_view_text + '</a>';
			...
			$.notify('&nbsp', {
				autoHideDelay: foodforlifeData.added_to_compare_notice.compare_notice_auto_hide,
				className: 'success',
				style: 'foodforlife',
				showAnimation: 'fadeIn',
				hideAnimation: 'fadeOut'
     ...
     * Copy link
     */
    foodforlife.copyLink = function () {
        $( '.foodforlife-copylink__button' ).on( 'click', function(e) {
            e.preventDefault();
            var $button = $(this).closest('form').find('.foodforlife-copylink__link');
            $button.select();
     ...
     * Currencies
     */
	foodforlife.currencyLanguage = function () {
		if( ! $('.foodforlife-currency-language').length ) {
			return;
		}

		$( document.body ).on( 'click', '.foodforlife-currency-language .current', function( event ) {
			...
			if ( $(this).hasClass('active') ) {
				...
				$('.foodforlife-currency-language .current').removeClass('active');
				$('.foodforlife-currency-language .currency-dropdown').removeClass('active');
				$('.foodforlife-currency-language-mobile-menu').removeClass('opened');

				$(this).next('.currency-dropdown').stop().toggleClass('active');
				$(this).toggleClass('active');

				if ( $(this).closest('.foodforlife-currency-language').hasClass('foodforlife-currency-language-mobile-menu') ) {
					$(this).closest('.foodforlife-currency-language-mobile-menu').toggleClass('opened');
				}
			}
		} ).on( 'keyup', function ( e ) {
			if ( e.keyCode === 27 ) {
			...
			if ( $target.is( '.foodforlife-currency-language .current' ) ) {
				return;
			}

			if ( $target.is( '.foodforlife-currency-language .preferences-menu__item-child' ) ) {
				return;
			}
		...
		$(document.body).on( 'foodforlife_popover_opened', function () {
			$('.foodforlife-currency-language .current').removeClass('active');
			$('.foodforlife-currency-language .currency-dropdown').removeClass('active');
			$('.foodforlife-currency-language-mobile-menu').removeClass('opened');
		});

		$(document.body).on( 'click', '.foodforlife-currency-language-mobile-menu .current', function() {
			var $currencyDropdown = $(this).next('.currency-dropdown');
			$(this).closest('.foodforlife-currency-language-mobile-menu').css('--ffl-currency-language-mobile-menu-height', $currencyDropdown.outerHeight() + 'px');
		});
    }

	foodforlife.mobileMenu = function () {
     ...
     */
	foodforlife.hamburgerToggleMenuItem = function () {
		var $menu  = $('#mobile-menu-panel'),
		...
		if( foodforlifeData.header_mobile_menu_open_primary_submenus_on == 'icon' ) {
		...
		$menu.find('li.menu-item-has-children').each(function() {
			if( ! $(this).find('ul.sub-menu > .submenu-items--heading').length ) {
		...
		$menu.on( 'click', $click, function( e ) {
			...
			if( $(this).closest('ul.sub-menu').length ) {
				$(this).closest('ul.sub-menu').scrollTop(0);
			}
			...
			$( this ).parents('ul.sub-menu').addClass('submenu--overflow').css( 'overflow', 'hidden' );
		...
		$menu.on( 'click', '.submenu-items--title', function( e ) {
			...
			if( $(this).closest('.mega-items--heading').length ) {
				...
				if ( $( this ).closest( 'li.sub-menu' ).hasClass('active') ) {
					$( this ).closest( 'li.sub-menu' ).removeClass('active');
				}
			} else {
				$( this ).closest('li.menu-item-has-children').removeClass('submenu--open');
			}

			$( this ).closest('.submenu--overflow').css( 'overflow', '' ).removeClass( 'submenu--overflow' );
		});
	}
     ...
     */
	foodforlife.hamburgerToggleMegaMenuItem = function () {
		var $menu  = $('#mobile-menu-panel'),
		...
		$menu.find(".mega-menu__column").each(function () {
			var $col = $(this);
			var $headings = $col.find('.menu-item--widget-heading');
			...
			$headings.each(function (index) {
				...
				if ($nextHeading.length) {
					$itemsBetween = $currentHeading.nextUntil($nextHeading, '.menu-item');
				} else {
					$itemsBetween = $currentHeading.nextAll('.menu-item');
				}

				if ($itemsBetween.length) {
					if( foodforlifeData.header_mobile_menu_open_primary_submenus_on == 'icon' ) {
						$currentHeading.children().append( '<span class="toggle-menu-children"></span>' );
					}

					let $wrapper = $("<li class='sub-menu'><ul></ul></li>");
					let $subMenu = $wrapper.find("ul");

					let $headingItem = $("<li class='submenu-items--heading mega-items--heading'>"
						+ $submenuHeading.html() +
						"</li>");

					$headingItem.appendTo($subMenu);

					$itemsBetween.appendTo($subMenu);

					$currentHeading.after($wrapper);
					$currentHeading.addClass('megamenu-item-has-children');
				}
			});
		});
		...
		$menu.on( 'click', $click, function( e ) {
			...
			if( foodforlifeData.header_mobile_menu_open_primary_submenus_on == 'icon' ) {
				$next = $(this).closest('.megamenu-item-has-children').next();
			}

			if ( ! $next.is( 'li.sub-menu' ) ) {
				return false;
			}

			if ( $next.is( 'li.sub-menu' ) && ! $next.hasClass('active') ) {
			...
			if( $(this).closest('.sub-menu').length ) {
				$(this).closest('.sub-menu').scrollTop(0);
			}
			...
			$( this ).parents('.sub-menu').addClass('submenu--overflow').css( 'overflow', 'hidden' );
     ...
     * Header campaign bar
     */
	foodforlife.headerCampaignBar = function () {
	 ...
	 * Sticky header
	 */
	foodforlife.stickyHeader = function () {
		if( foodforlife.$body.hasClass('elementor-editor-active') ) {
			return;
		}
		if( foodforlifeData.header_sticky ) {
			sticky( foodforlife.$header.find('.site-header__desktop'), foodforlife.$header.find('.site-header__section.foodforlife-header-sticky') );
		}

		if( foodforlifeData.header_mobile_sticky ) {
			sticky( foodforlife.$header.find('.site-header__mobile'), foodforlife.$header.find('.site-header__section.foodforlife-header-mobile-sticky') );
		}

		function sticky( $headerSticky, $headerSection ) {
			var header    = foodforlife.$header.outerHeight(true) / 2,
				hBody     = foodforlife.$body.outerHeight(true),
				...
				heightHeader = foodforlife.$header.find('.site-header__desktop').length ? foodforlife.$header.find('.site-header__desktop').outerHeight() : 0,
				heightHeaderMobile = foodforlife.$header.find('.site-header__mobile').length ? foodforlife.$header.find('.site-header__mobile').outerHeight() : 0;

			if( hBody < scrollTop*5 ) {
				return;
			}

			if ( 'up' === foodforlifeData.header_sticky_on ) {
			...
			foodforlife.$window.on('scroll', function () {
				var scroll = foodforlife.$window.scrollTop();

				if (hBody <= scrollTop + foodforlife.$window.height()) {
					return;
				}


				if (scroll > scrollTop) {
					foodforlife.$header.css({
						'--ffl-header-height': heightHeader + 'px',
						'--ffl-header-height-mobile': heightHeaderMobile + 'px'
					});
					...
					foodforlife.$header.addClass('ffl-header-sticky-visible');

				} else {
					foodforlife.$header.removeAttr('style');
					foodforlife.$header.removeClass('ffl-header-sticky-visible');
	...
	foodforlife.headerProductCategories = function () {
		var $menu = $('#header-sidebar-categories'),
			$backdrop = $menu.find('.header-sidebar-categories__backdrop');

		if ( 'hover' === foodforlifeData.header_sidebar_categories_action ) {
			$menu.on('mouseenter', '.header-sidebar-categories__header', function () {
			...
			$(document).on('click', '.header-sidebar-categories__header', function (e) {
				$menu.addClass('opened');
			});
	 ...
	 * Back to top icon
	 */
	foodforlife.backToTop = function () {
		var $scrollTop = $('#gotop');

		foodforlife.$window.on('scroll', function () {
			if ( foodforlife.$window.scrollTop() > 100 ) {
				$scrollTop.addClass('show-scroll');
			} else {
				$scrollTop.removeClass('show-scroll');
			}

			let scrollTop2    = foodforlife.$window.scrollTop(),
				docHeight     = $(document).height(),
				winHeight     = $(window).height(),
				scrollPercent = (scrollTop2 / (docHeight - winHeight)) * 100;

			$scrollTop.css( '--ffl-gotop-height-scroll', scrollPercent + '%' );
		});

		foodforlife.$body.on('click', '#gotop', function (e) {
			e.preventDefault();
	 ...
	 * @return boolean
	 */
	foodforlife.toggleProductCategoriesWidget = function() {
		var $widget = $( '.widget_product_categories' );

		$widget.find('li.cat-parent').each( function() {
			if ( $(this).find('ul.children').length > 0 ) {
				$(this).append('<span class="ffl-product-cat-item-toggle"></span>');
			}
		});

		$widget.on( 'click', 'li.cat-parent > .ffl-product-cat-item-toggle', function (e) {
			e.preventDefault();
		...
		$('.catalog-filters-sidebar .wp-block-group').each(function(){
			if( $(this).find('.wp-block-heading').length ) {
				$(this).find('.wp-block-heading').addClass('ffl-widget-heading clicked');
				$(this).find('.wp-block-heading').append('<span class="ffl-collapse-icon"></span>');
				$(this).find('.wp-block-heading').nextAll().wrapAll('<div class="ffl-widget-group dropdown"/>');
			}
		});

		$('.catalog-filters-sidebar .widget').each(function(){
			if( $(this).find('.widget-title').length ) {
				$(this).find('.widget-title').addClass('ffl-widget-heading clicked');
				$(this).find('.widget-title').append('<span class="ffl-collapse-icon"></span>');
				$(this).find('.widget-title').nextAll().wrapAll('<div class="ffl-widget-group dropdown"/>');
			}
		});

		$('.catalog-filters-sidebar .ffl-widget-heading').on('click', function (e) {
			e.preventDefault();
	 ...
	 * @return boolean
	 */
	foodforlife.dropdownProductCategoriesSidebar = function( el ) {
		...
		$widget.addClass('ffl-product-categories-widget');

		$categoriesList.addClass('ffl-product-categories-dropdown');
		$categoriesList.closest('.wc-block-product-categories-list-item').addClass('ffl-product-categories-has-children');
		$categoriesList.closest('.wc-block-product-categories-list-item').append('<span class="ffl-product-categories-toggler" aria-hidden="true"></span>');

		$widget.on( 'click', '.ffl-product-categories-has-children > .ffl-product-categories-toggler', function (e) {
			e.preventDefault();

			var $item = $( this ).closest('.ffl-product-categories-has-children');

			$item.toggleClass( 'active' ).siblings().removeClass( 'active' );

			// If this is sub-menu item
			$item.children( '.ffl-product-categories-dropdown' ).slideToggle();
			$item.siblings().find( '.ffl-product-categories-dropdown' ).slideUp();

        });
	};

	foodforlife.formatPrice = function( $number ) {
		var currency       = foodforlifeData.currency_symbol,
			thousand       = foodforlifeData.thousand_sep,
			decimal        = foodforlifeData.decimal_sep,
			price_decimals = foodforlifeData.price_decimals,
			currency_pos   = foodforlifeData.currency_pos,
			n              = $number;
	 ...
	 * Tooltip
	 */
	foodforlife.tooltip = function () {
		foodforlife.$body.on( 'mouseenter', '.ffl-tooltip', function (e) {
			tooltip_data($(this));
		}).on( 'mouseleave', '.ffl-tooltip', function (e) {
			var tooltip = foodforlife.$body.find('.ffl-tooltip--data');
		...
		function tooltip_data($el) {
			if( $el.hasClass( 'loading') ) {
				$('.ffl-tooltip--data').remove();
				return;
			}

			var $tooltip = $( '<div class="ffl-tooltip--data position-absolute text-light"></div>' ),
				self = $el,
				time = 50;
			...
			$tooltip.fadeIn( time, function() {
				var position = self.offset(),
					...
					tooltipWidth = $('.ffl-tooltip--data').outerWidth(),
					tooltipHeight = $('.ffl-tooltip--data').outerHeight(),
					top = ( position.top - tooltipHeight ) + height + 'px',
					left = position.left + self.outerWidth() / 2 - tooltipWidth / 2 + 'px',
					spacing = 10;

				if ( $el.hasClass('ffl-tooltip-short-spacing') ) {
					spacing = 6;
				}
	...
	foodforlife.orderComments = function() {
	...
	foodforlife.navigationBar = function() {
		var $navigationBar = $('#foodforlife-mobile-navigation-bar');

		if ( ! $navigationBar.length ) {
			return;
		}

		foodforlife.$body.css('--ffl-navigation-bar-height', $navigationBar.outerHeight() + 'px');

		foodforlife.$body.addClass('mobile-has-navigation-bar');
	}
