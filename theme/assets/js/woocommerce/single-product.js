(function ($) {
	...
	foodforlife.init = function () {
		...
		foodforlife.$body.on( 'foodforlife_product_gallery_zoom', function(){
			foodforlife.productImageZoom();
		} );

		this.productLightBox();
		this.foodforlifeMore();
		this.productReview();
	 ...
	 * Product Gallery
	 */
	foodforlife.productGallery = function ( vertical, $selector = $('.woocommerce-product-gallery') ) {
			...
			var options = {
				...
				on: {
					init: function () {
						...
						foodforlife.$body.trigger( 'foodforlife_product_gallery_init' );
					...
					slideChangeTransitionEnd: function () {
						foodforlife.$body.trigger( 'foodforlife_product_gallery_slideChangeTransitionEnd' );
					}
				}
			};
			...
			var options = {
				...
				on: {
					init: function () {
						...
						foodforlife.$body.trigger( 'foodforlife_product_thumbnails_init' );
					},
				},
			};
	 ...
	 */
	foodforlife.singleProductGallery = function () {
		...
		if( $gallery.hasClass( 'woocommerce-product-gallery--vertical' ) ) {
			$('.woocommerce-product-gallery').on('product_thumbnails_slider_vertical wc-product-gallery-after-init', function(){
				foodforlife.productGallery(true);
			});
		}

		if( $gallery.hasClass( 'woocommerce-product-gallery--horizontal' ) ) {
			foodforlife.productGallery(false);
	 ...
	 * Product Image Zoom
	 */
	foodforlife.productImageZoom = function () {
		...
		if( foodforlifeData.product_image_zoom == 'none' ) {
			return;
		}
		...
		if( foodforlifeData.product_image_zoom == 'bounding' ) {
			if( ! $summary.find('.foodforlife-product-zoom-wrapper').length ) {
				var $zoom = $( '<div class="foodforlife-product-zoom-wrapper" />' );
		...
		if( foodforlifeData.product_image_zoom == 'inner' ) {
			options.zoomFactor = 3;
		}

		if( foodforlifeData.product_image_zoom == 'magnifier' ) {
			options.zoomFactor = 2;
		...
		$gallery.find( '.woocommerce-product-gallery__image' ).each( function() {
			...
			if( $this.hasClass('foodforlife-product-video') || $this.data( 'zoom_status' ) == false ) {
				return;
			}
	...
	/**
 	 * FoodForLife More
 	 */
	foodforlife.foodforlifeMore = function () {
	 ...
	 * Product Light Box
	 */
	foodforlife.productLightBox = function () {
		var $selector = $('.woocommerce-product-gallery');

		$( '.woocommerce-product-gallery.woocommerce-product-gallery--has-zoom .woocommerce-product-gallery__image:not(.foodforlife-product-3d-viewer)' ).on( 'click', function (e) {
			e.preventDefault();

			initLightBox( $(this).find( 'a' ) );
		});
		...
		if( ! foodforlifeData.product_image_lightbox ) {
			return
		}
		lightBoxButton();
		foodforlife.$body.on( 'foodforlife_product_gallery_lightbox', function(){
			lightBoxButton();
		} );

		$(document).on( 'click', '.foodforlife-button--product-lightbox', function (e) {
			e.preventDefault();
		...
		function initLightBox( $selector) {
			var pswpElement = $( '.pswp' )[0],
				items       = getGalleryItems( $selector.hasClass( 'foodforlife-button--product-lightbox' ) ? $selector.siblings( '.woocommerce-product-gallery__wrapper' ).find( '.woocommerce-product-gallery__image' ) : $selector.closest( '.woocommerce-product-gallery__wrapper' ).find( '.woocommerce-product-gallery__image' ) ),
				clicked = $selector.hasClass( 'foodforlife-button--product-lightbox' ) ? $selector.siblings( '.woocommerce-product-gallery__wrapper' ).find( '.swiper-slide-active' ) : $selector.closest( '.woocommerce-product-gallery__image' ),
				index = 0;
		...
		function getGalleryItems( $slides ) {
			...
			if ( $slides.length > 0 ) {
				$slides.each( function( i, el ) {
					...
					if ( $( el ).hasClass('foodforlife-product-video') ) {
						var video = $( el ).find('.foodforlife-video-wrapper').html();
	 ...
	 * Product Review
	 */
	foodforlife.productReview = function () {
		var $button = $( '.foodforlife-form-review' ),
			text = $button.data( 'text' ),
			textCancel = $button.data( 'text-cancel' ),
			$form = $( '.foodforlife-review-form');
	 ...
	 */
	foodforlife.productTabsDropdown = function () {
		...
		if( foodforlifeData.product_tabs_layout !== 'accordion' ) {
			return;
		}
		...
		$productTabs.on( 'click', '.woocommerce-tabs-title', function() {
			if( $(this).hasClass('active') ) {
				...
				if ( foodforlifeData.product_tabs_single_open ) {
					$(this).closest('.woocommerce-tabs--dropdown').siblings('.woocommerce-tabs--dropdown').find('.woocommerce-tabs-title.active').trigger('click');
				}
			}
		});
	 ...
	 */
	foodforlife.addToCartGroupedProductVariable = function () {
		if( ! foodforlife.$body.hasClass( 'single-product' ) ) {
			return;
		}

		if( ! foodforlife.$body.find( '.woocommerce-grouped-product-list' ).length ) {
			return;
		}
	 ...
	 * Product Highlights
	 */
	foodforlife.productHighlights = function () {
		if( ! foodforlifeData.product_highlights ) {
			return;
		}
		...
		var $inner = $selector.find('.foodforlife-marquee__inner'),
			$items = $selector.find('.foodforlife-marquee__items');
