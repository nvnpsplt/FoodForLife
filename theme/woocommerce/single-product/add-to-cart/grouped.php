				...
				<div class="woocommerce-grouped-product-list__summary flex-1">
					...
					<?php
						if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
							if( $grouped_product_child->is_type( 'variable' ) && class_exists( 'FoodForLife\Addons\Modules\Base\Variation_Select' ) ) {
								echo \FoodForLife\Addons\Modules\Base\Variation_Select::instance()->render($grouped_product_child);
							...
							echo '<label for="' . esc_attr( 'quantity-' . $grouped_product_child->get_id() ) . '" class="screen-reader-text">' . esc_html__( 'Buy one of this item', 'foodforlife' ) . '</label>';
						} else {
							woocommerce_quantity_input(
								array(
									'input_name'  => 'quantity[' . $grouped_product_child->get_id() . ']',
