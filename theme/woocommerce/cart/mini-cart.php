...
<?php if ( WC()->cart && ! WC()->cart->is_empty() ) : ?>
	...
	<?php do_action( 'foodforlife_before_woocommerce_mini_cart_items' ); ?>
					...
					<div class="woocommerce-mini-cart-item__summary">
						<div class="woocommerce-mini-cart-item__box">
							<span class="woocommerce-mini-cart-item__data">
								...
								<?php endif; ?>
								<?php if( $unit_measure ) : ?>
									<span class="ffl-price-unit"><span class="divider">/</span> <?php echo esc_html( $unit_measure ); ?></span>
									</div>
								<?php endif; ?>
								<span class="woocommerce-mini-cart-item__qty--text hidden">
									<?php esc_html_e( 'QTY:', 'foodforlife' ); ?>
									<?php echo wp_kses_post( $cart_item['quantity'] ); ?>
								</span>
							</span>
							<span class="woocommerce-mini-cart-item__qty" data-nonce="<?php echo wp_create_nonce( 'foodforlife-update-cart-qty--' . $cart_item_key ); ?>">
								...
								<span class="woocommerce-mini-cart-item__actions d-flex flex-column ms-10">
									...
									if( empty( $matchingKeys ) && $_product->get_type() === 'variation' ) {
										echo apply_filters(
											...
											sprintf(
												'<a href="%s" class="foodforlife-quickedit-button ffl-button ffl-button-text" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-position="%s">%s</a>',
												esc_url( wc_get_cart_remove_url( $cart_item_key ) ),
												esc_attr__( 'Update this item', 'foodforlife' ),
												esc_attr( $product_id ),
												esc_attr( $cart_item_key ),
												esc_attr( $position ),
												\FoodForLife\Icon::get_svg('edit', 'ui', array( 'class' => 'icon-fill-none' )),
											),
											$cart_item_key
										);
									}
									...
									<?php
										echo apply_filters(
											...
											sprintf(
												'<a href="%s" class="remove ffl-button ffl-button-text remove_from_cart_button" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s" data-success_message="%s">%s</a>',
												esc_url( wc_get_cart_remove_url( $cart_item_key ) ),
												esc_attr__( 'Remove this item', 'foodforlife' ),
												esc_attr( $product_id ),
												esc_attr( $cart_item_key ),
												esc_attr( $_product->get_sku() ),
												esc_attr( sprintf( __( '&ldquo;%s&rdquo; has been removed from your cart', 'woocommerce' ), wp_strip_all_tags( $product_name ) ) ),
												\FoodForLife\Icon::inline_svg( 'icon=trash'),
											),
											$cart_item_key
										);
									?>
	...
	<?php do_action( 'foodforlife_after_woocommerce_mini_cart_items' ); ?>

	<div class="widget_shopping_cart_footer">
		<?php do_action( 'foodforlife_before_widget_shopping_cart_total' ); ?>
...
<?php else : ?>
	...
	<div class="woocommerce-mini-cart__empty-message">
		<span class="ffl-svg-icon ffl-svg-icon--cart">
		...
		<div class="fs-18 mb-20 mt-15 h4"><?php echo esc_html__( 'Your cart is currently empty!.', 'foodforlife' );?></div>
		<p class="mb-20"><?php echo esc_html__( 'You may check out all the available products and buy some in the shop.', 'foodforlife' );?></p>
		<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ) ?>" class="ffl-button ffl-button-hover-effect"><?php echo esc_html__( 'Continue Shopping', 'foodforlife' );?></a>
	</div>
