...
class Checkout_Page {
	...
	public function __construct() {
		...
		add_action( 'foodforlife_woocommerce_product_archive_content', array( $this, 'checkout_content_builder' ), 5 );
	...
	public function body_classes( $classes ) {
		...
		if( is_singular( 'foodforlife_builder' ) && ( in_array( 'checkout_page', $terms ) ) ) {
			$classes[] = 'foodforlife-woocommerce-elementor checkout-page-elementor woocommerce woocommerce-checkout woocommerce-page foodforlife-elementor-id-'.self::$page_id;
		} else {
			$classes[] = 'foodforlife-woocommerce-elementor checkout-page-elementor foodforlife-elementor-id-'.self::$page_id;
		}
	...
	public function enqueue_scripts() {
		if( ! apply_filters( 'foodforlife_get_checkout_page_builder', true ) ) {
			return;
		}
    ...
    public function get_content_empty_html() {
        ?>
            <div class="foodforlife-single-product-builder--empty">
                <h4><?php esc_html_e( 'Checkout Page Builder', 'foodforlife-addons' ); ?></h4>
                <?php
                    printf(
                        esc_html__( "It seems like you've turned on the Checkout Page Builder, but you haven't set up any builders yet. To avoid any issues, please either %s this feature or %s. You can find a step-by-step guide in our documentation.", 'foodforlife-addons' ),
                        sprintf(
                            '<a href="%s">%s</a>',
	...
	public function foodforlife_post_state( $post_states, $post ) {
		...
		if( ! get_option( 'foodforlife_checkout_page_builder_enable') ) {
			return $post_states;
		}
		...
		if ( get_the_ID() == wc_get_page_id( 'checkout' ) ) {
			...
			$post_states['foodforlife_builder'] = esc_html__( 'FoodForLife Checkout Builder', 'foodforlife-addons' );
		}
