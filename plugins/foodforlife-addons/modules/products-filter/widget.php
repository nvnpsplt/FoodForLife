...
class Widget extends \WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'foodforlife-products-filter',
			esc_html__( 'FoodForLife - Products Filter', 'foodforlife-addons' ),
			array(
				'classname'                   => 'products-filter-widget woocommerce',
				'description'                 => esc_html__( 'WooCommerce products filter.', 'foodforlife-addons' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( $title = apply_filters( 'widget_title', $instance['title'], $instance, $this->id_base ) ) {
			echo $args['before_title'] . \FoodForLife\Addons\Helper::get_svg( 'filter' ) . esc_html( $title ) . $args['after_title'];
		}
		...
		if ( ! empty( $instance['filter'] ) ) {
			...
			echo '<h6>' . esc_html__( 'Refine by', 'foodforlife-addons' ) . '</h6>';
			echo '<button type="reset" value="' . esc_attr__( 'Clear All', 'foodforlife-addons' ) . '" class="foodforlife-button--subtle alt reset-button">' . esc_html__( 'Clear All', 'foodforlife-addons' ) . '</button>';
			echo '</div>';
			...
			echo '<button type="submit" value="' . esc_attr__( 'Filter', 'foodforlife-addons' ) . '" class="button filter-button foodforlife-button--bg-color-black foodforlife-button--base">' . esc_html__( 'Apply', 'foodforlife-addons' ) . '</button>';
			echo '<button type="reset" value="' . esc_attr__( 'Reset Filter', 'foodforlife-addons' ) . '" class="button ffl-button-subtle alt reset-button fs-16">' . esc_html__( 'Clear All', 'foodforlife-addons' ) . '</button>';
			echo '</div>';
		...
		if ( ! empty( $filter['show_view_more'] ) && empty( $filter['scrollable'] ) && in_array( $filter['display'], array( 'list', 'checkboxes' ) ) ) {
			$view_more = sprintf(
				'<div class="foodforlife-widget-product-cats-btn">
					<span class="ffl-button-subtle show-more">%s</span>
					<span class="ffl-button-subtle show-less">%s</span>
		...
		switch ( $filter['source'] ) {
			...
			case 'products_group':
				$filter_groups = apply_filters( 'foodforlife_products_filter_groups', array(
					'best_sellers' => esc_attr__( 'Best Sellers', 'foodforlife-addons' ),
	...
	public function form( $instance ) {
		...
		<div class="foodforlife-products-filter-form__sub-nav">
			<button type="button" data-section="filters" class="button-link active"><?php esc_html_e( 'Filters', 'foodforlife-addons' ); ?></button> |
			<!-- <button type="button" data-section="design" class="button-link"><?php esc_html_e( 'Design', 'foodforlife-addons' ); ?></button> | -->
			<button type="button" data-section="options" class="button-link"><?php esc_html_e( 'Options', 'foodforlife-addons' ); ?></button>
		</div>

		<p><hr/></p>

		<div class="foodforlife-products-filter-form__section active" data-section="filters">
			<p class="foodforlife-products-filter-form__message <?php echo ! empty( $instance['filter'] ) ? 'hidden' : '' ?>"><?php esc_html_e( 'There is no filter yet.', 'foodforlife-addons' ) ?></p>

			<div class="foodforlife-products-filter-form__filter-fields">
				<?php $this->filter_setting_fields( $instance['filter'] ); ?>
			</div>

			<p class="foodforlife-products-filter-form__section-actions">
				<button type="button" class="foodforlife-products-filter-form__add-new button-link" data-name="<?php echo esc_attr( $this->get_field_name( 'filter' ) ); ?>" data-count="<?php echo count( $instance['filter'] ) ?>">+ <?php esc_html_e( 'Add a new filter', 'foodforlife-addons' ) ?></button>
			</p>
		</div>

		<div class="foodforlife-products-filter-form__section" data-section="design">
		</div>

		<div class="foodforlife-products-filter-form__section" data-section="options">
			<?php
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'ajax',
				'label' => esc_html__( 'Use ajax for filtering', 'foodforlife-addons' ),
				'value' => $instance['ajax'],
			) );
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'instant',
				'label' => esc_html__( 'Filtering products instantly (no buttons required)', 'foodforlife-addons' ),
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'change_url',
				'label' => esc_html__( 'Update URL', 'foodforlife-addons' ),
			...
			$this->setting_field( array(
				...
				'label' => esc_html__( 'Filter Sections on Mobile', 'foodforlife-addons' ),
				'options' => array(
					'collapse' => esc_html__( 'Collapse', 'foodforlife-addons' ),
					'expand' => esc_html__( 'Expand', 'foodforlife-addons' ),
				),
	...
	protected function filter_setting_fields( $fields = array(), $context = 'display' ) {
		...
		foreach ( $filter_fields as $index => $field ) :
			...
			?>
			<div class="foodforlife-products-filter-form__filter">
				<div class="foodforlife-products-filter-form__filter-top">
					<div class="foodforlife-products-filter-form__sort">
						<button type="button" class="foodforlife-products-filter-form__sort-up button-link button-link-sort">
							<span class="foodforlife-products-filter-form__sort-up-icon" aria-hidden="true"></span>
						</button>
						<button type="button" class="foodforlife-products-filter-form__sort-down button-link button-link-sort">
							<span class="foodforlife-products-filter-form__sort-down-icon" aria-hidden="true"></span>
						</button>
					</div>
					<div class="foodforlife-products-filter-form__filter-actions">
						<button type="button" class="foodforlife-products-filter-form__remove-filter button-link button-link-delete">
							<span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'foodforlife-addons' ) ?></span>
							<span class="dashicons dashicons-no-alt"></span>
						</button>
					</div>

					<button type="button" class="foodforlife-products-filter-form__filter-toggle">
						<span class="foodforlife-products-filter-form__filter-toggle-indicator" aria-hidden="true"></span>
					</button>

					<div class="foodforlife-products-filter-form__filter-title"><?php echo $title; ?></div>
				</div>
				<div class="foodforlife-products-filter-form__filter-options">
					<?php
					foreach ( $filter_settings as $name => $options ) {
						...
						$options['class'] = 'foodforlife-products-filter-form__filter-option';
	...
	protected function get_filter_fields_settings() {
		...
		$this->filter_settings = array(
			...
			'ranges' => array(
				...
				'desc' => __( 'Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol.', 'foodforlife-addons' ),
				'condition' => array(
					'display' => 'ranges',
					'source'  => 'price',
				),
			...
			'include_categories' => array(
				'type' => 'text',
				'label' => __( 'Include Categories (IDs separated by commas. Eg: 1,2,3,..)', 'foodforlife-addons' ),
				'condition' => array(
					'source' => 'product_cat',
				)
			),
			'exclude_categories' => array(
				'type' => 'text',
				'label' => __( 'Exclude Categories (IDs separated by commas. Eg: 1,2,3,..)', 'foodforlife-addons' ),
				'condition' => array(
					'source' => 'product_cat',
				)
			),
			...
			'searchable' => array(
				'type' => 'checkbox',
				'label' => __( 'Show the search box', 'foodforlife-addons' ),
				'condition' => array(
			...
			'show_empty_brands'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show empty brands', 'foodforlife-addons' ),
				'condition' => array(
			...
			'show_empty_attributes'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show empty attributes', 'foodforlife-addons' ),
				'condition' => array(
			...
			'cats_numbers'                   => array(
				'type'  => 'number',
				'label' => esc_html__( 'Categories per view', 'foodforlife-addons' ),
				'condition' => array(
					'source'  => array( 'product_cat' ),
					'display' => array( 'list', 'checkboxes' ),
					'show_view_more!' => '',
			...
			'disable_onsale'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable on sale', 'foodforlife-addons' ),
				'condition' => array(
			...
			'disable_in_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable in stock', 'foodforlife-addons' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
			'disable_out_of_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable out of stock', 'foodforlife-addons' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
	...
	public function admin_scripts( $hook ) {
		...
		wp_enqueue_style( 'foodforlife-products-filter-admin', FOODFORLIFE_ADDONS_URL . 'modules/products-filter/assets/admin/products-filter-admin.css', array(), '20210311' );
		wp_enqueue_script( 'foodforlife-products-filter-admin', FOODFORLIFE_ADDONS_URL . 'modules/products-filter/assets/admin/products-filter-admin.js', array( 'wp-util', 'jquery-ui-sortable' ), '20210311', true );

		wp_localize_script(
			'foodforlife-products-filter-admin', 'foodforlife_products_filter_params', array(
				'sources'    => $this->get_filter_source_options(),
	...
	public function scripts() {
		...
		wp_enqueue_script( 'foodforlife-products-filter', FOODFORLIFE_ADDONS_URL . 'modules/products-filter/assets/products-filter' . $debug . '.js', array(
			'jquery',
			'wp-util',
			'select2',
	...
	public function filter_setting_fields_template() {
        ...
        <script type="text/template" id="tmpl-foodforlife-products-filter">
	...
	public function product_status($query ) {
		...
		$product_status = explode(',', sanitize_text_field( wp_unslash( $_GET['product_status'] ) ) ); // S-ADDON: Sanitize.

		$meta_query = array(); // S-ADDON: Initialize to prevent PHP warning.
