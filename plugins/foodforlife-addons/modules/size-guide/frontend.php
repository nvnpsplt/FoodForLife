...
class Frontend {
	...
	public function __construct() {
		...
		add_action( 'foodforlife_before_single_product', array( $this, 'display_size_guide'  ) );
	...
	public function scripts() {
		wp_enqueue_style( 'foodforlife-size-guide-content', FOODFORLIFE_ADDONS_URL . 'modules/size-guide/assets/css/size-guide.css', array(), '1.0' );
		wp_enqueue_script('foodforlife-size-guide-content', FOODFORLIFE_ADDONS_URL . 'modules/size-guide/assets/js/size-guide-tab.js');
	}
	...
	public function display_size_guide() {
		...
		$guide_settings = get_post_meta( $product->get_id(), 'foodforlife_size_guide', true );
		...
		if ( 'tab' == $display ) {
			...
			switch ( $button_position ) {
				case 'bellow_summary':
					...
					add_action( 'foodforlife_woocommerce_product_quickview_summary', array( $this, 'size_guide_button' ), 32 );
					add_action( 'woocommerce_after_single_product_summary', array( $this, 'size_guide_panel' ), 12 );
					add_action( 'foodforlife_woocommerce_after_product_quickview_summary', array( $this, 'size_guide_panel' ), 12 );
					add_action( 'foodforlife_footer_elementor', array( $this, 'size_guide_panel' ), 12 );
					break;

				case 'bellow_price':
					add_action( 'woocommerce_single_product_summary', array( $this, 'size_guide_button' ), 12 );
					add_action( 'foodforlife_woocommerce_product_quickview_summary', array( $this, 'size_guide_button' ), 29 );
					add_action( 'woocommerce_after_single_product_summary', array( $this, 'size_guide_panel' ), 12 );
					add_action( 'foodforlife_woocommerce_after_product_quickview_summary', array( $this, 'size_guide_panel' ), 12 );
					add_action( 'foodforlife_footer_elementor', array( $this, 'size_guide_panel' ), 12 );
					break;
	...
	public function size_guide_tab( $tabs ) {
		...
		$button_text = $text ? $text : esc_html__('Size Guide' , 'foodforlife-addons');
	...
	protected function get_size_guide_button() {
		...
		$button_text = esc_html__('Size guide' , 'foodforlife-addons');
			...
			<div class="modal__container">
				<div class="modal__wrapper">
					<div class="modal__header">
						<h3 class="modal__title h5"><?php esc_html_e( 'Size Guide', 'foodforlife-addons' ) ?></h3>
	...
	public function size_guide_content() {
		...
		echo '<div class="foodforlife-size-guide">';

		if ( ! empty( $guide->post_content ) ) {
			echo '<div class="foodforlife-size-guide--global-content">' . $this->kses_content( $guide->post_content, $guide ) . '</div>';
		}
		...
		if ( 1 < count( $size_guides['tables'] ) ) {
			...
			echo '<div class="foodforlife-size-guide-tabs">';
			echo '<ul class="foodforlife-size-guide-tabs__nav">' . implode( '', $tabs ) . '</ul>';
			echo '<div class="foodforlife-size-guide-tabs__panels">';
		}

		foreach ( $size_guides['tables'] as $index => $table ) {
			echo '<div class="foodforlife-size-guide-tabs__panel ' . ( $index ? '' : 'active' ) . '" data-panel="' . esc_attr( $index + 1 ) . '">';

			if ( ! empty( $size_guides['names'][ $index ] ) ) {
				echo '<h4 class="foodforlife-size-guide__name h6">' . wp_kses_post( $size_guides['names'][ $index ] ) . '</h4>';
			}

			if ( ! empty( $table ) ) {
				$table = json_decode( $table, true );

				echo '<div class="foodforlife-size-guide__table-wrapper">';
				echo '<table class="foodforlife-size-guide__table">';
			...
			if ( ! empty( $size_guides['information'][ $index ] ) ) {
				echo '<div class="foodforlife-size-guide__info">' . wp_kses_post( $size_guides['information'][ $index ] ) . '</div>';
			}

			if ( ! empty( $size_guides['descriptions'][ $index ] ) ) {
				echo '<div class="foodforlife-size-guide__description">' . wp_kses_post( $size_guides['descriptions'][ $index ] ) . '</div>';
			}
	...
	public function get_product_size_guide_id( $object = false ) {
		...
		$size_guide = get_post_meta( $_product->get_id(), 'foodforlife_size_guide', true );
