...
class Product_Meta  {
	...
	const TAXONOMY_TYPE   = 'foodforlife_product_tab_type';
	...
	public function product_data_tab( $tabs ) {
		...
		$tabs['foodforlife_product_tabs'] = array(
			'label'    => esc_html__( 'Product Tabs', 'foodforlife-addons' ),
			'target'   => 'foodforlife-product-tabs',
			'class'    => array( 'foodforlife-product-tabs', ),
			'priority' => 62,
		);
	...
	public function product_data_panel() {
		...
		global $post;
		?>
		<div id="foodforlife-product-tabs" class="panel wc-metaboxes-wrapper woocommerce_options_panel">
			<div class="wc-metaboxes">
				...
				<?php
					...
					$tab_title = $tab_disable ? $tab_title . '(' . esc_html__( 'Disabled', 'foodforlife-addons' ) . ')' : $tab_title;
				...
				<div class="wc-metabox postbox">
					<h3 class="<?php echo esc_attr($tab_class); ?>" style="border-bottom: 1px solid #ccc">
						<div class="handlediv" title="<?php esc_attr_e('Click to toggle', 'foodforlife-addons'); ?>"></div>
					...
					<div class="wc-metabox-content" style="padding: 20px">
						...
						woocommerce_wp_checkbox(
							array(
								...
								'label'       => esc_html__('Tab Disable', 'foodforlife-addons' ),
								'value'       => wc_bool_to_string( $tab_disable ),
							)
						);?>
						<fieldset class="form-field">
							<label><?php esc_html_e( 'Tab Content', 'foodforlife-addons' ); ?></label>
							<?php
	...
	public function get_product_tabs() {
		...
		$products = new \WP_Query( array(
			...
			'posts_per_page' => apply_filters( 'foodforlife_addons_product_tabs_query_limit', 50 ),
			'fields'         => 'ids',
			'no_found_rows'  => true,
			'orderby' 		=> 'menu_order',
			'order' 		=> 'DESC',
		...
		if(  $this->product_tab_ids ) {
			foreach( $this->product_tab_ids as $tab_id ) {
				if ( isset($_POST['_product_tab_content_' . $tab_id] ) && ! empty( $_POST['_product_tab_content_' . $tab_id] ) ) {
					update_post_meta( $post_id, '_product_tab_content_' . $tab_id, wp_kses_post( wp_unslash( $_POST['_product_tab_content_' . $tab_id] ) ) ); // S-ADDON: Sanitize.
				}
