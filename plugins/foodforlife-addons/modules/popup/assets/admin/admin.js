(function($) {
	$(function() {
		$(".foodforlife-popup__toggle-button").on("change", '.foodforlife-popup__enabled', function(e) {
			e.preventDefault();
			var $button = $(this),
				newState = 0,
			...
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: "foodforlife_save_popup_enable",
					nonce: nonce,
					post_ID: post_ID,
					enabled: newState
				}
		...
		if ( $('.foodforlife-popup__visible').val() == 'delay' ) {
			$('.foodforlife-popup__visible').next().removeClass('hidden');
		}

		$(".foodforlife-popup-triggers").on("change", '.foodforlife-popup__visible', function(e) {
		...
		$( 'table.widefat tbody' ).sortable({
			...
			update: function( event, ui ) {
				...
				$.post(
					ajaxurl,
					{ action: 'foodforlife_popup_ordering', id: postid, previd: prevpostid, nextid: nextpostid },
					function( response ) {
