...
class Frontend {
	...
	public function __construct() {
		...
		add_filter(	'foodforlife_get_cart_svg_icon', array( $this, 'change_cart_svg_icon' ) );
		add_action( 'foodforlife_before_site', array( $this, 'include_svg_icons' ) );

		add_action( 'foodforlife_after_site_content_open', array( $this, 'open_site_content_container' ) );
		add_action( 'foodforlife_before_site_content_close', array( $this, 'close_site_content_container' ), 30 );
	...
	public function content_layout() {
		...
		return apply_filters( 'foodforlife_site_layout', $layout );
	}
	...
	public function open_site_content_container() {
		...
		$classes = apply_filters( 'foodforlife_site_content_container_class', 'container-xxl' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style('foodforlife-base', get_template_directory_uri() . '/assets/css/base'. $debug . '.css', array(), \FoodForLife\Helper::get_theme_version());
		wp_enqueue_style( 'foodforlife', apply_filters( 'foodforlife_get_style_directory_uri', get_template_directory_uri() ) . '/' . 'style.css',
			array(
				'swiper',
			),
		...
		if ( is_rtl() ) {
			wp_enqueue_style('foodforlife-rtl', get_template_directory_uri() . '/rtl.css', array(), \FoodForLife\Helper::get_theme_version());
		}

		do_action( 'foodforlife_after_enqueue_style' );
		...
		if ( ( \FoodForLife\Helper::get_option( 'header_sticky' ) || \FoodForLife\Helper::get_option( 'header_mobile_sticky' ) ) && 'up' == \FoodForLife\Helper::get_option( 'header_sticky_on' ) ) {
			wp_enqueue_script( 'headroom' );
		}

		wp_register_script( 'foodforlife-countdown',  get_template_directory_uri() . '/assets/js/plugins/jquery.countdown.js', array(), '1.0', array('strategy' => 'defer' ));

		wp_register_script( 'foodforlife-visibility',  get_template_directory_uri() . '/assets/js/plugins/visibility.js', array(), '1.0', array('strategy' => 'defer' ) );

		wp_register_script( 'notify', get_template_directory_uri() . '/assets/js/plugins/notify.min.js', array(), '1.0.0', array('strategy' => 'defer' ));
		wp_enqueue_script( 'foodforlife', get_template_directory_uri() . "/assets/js/scripts" . $debug . ".js",
			array(
				'jquery',
				'swiper',
				'imagesloaded',
				'foodforlife-visibility',
			),
			\FoodForLife\Helper::get_theme_version(),
		...
		$foodforlife_data = array(
			...
			'recently_viewed_enable'               => get_option( 'foodforlife_recently_viewed_enable', 'yes' ),
			'header_sidebar_categories_action' => Helper::get_option( 'header_sidebar_categories_action' ),
		);

		$foodforlife_data = apply_filters( 'foodforlife_wp_script_data', $foodforlife_data );

		wp_localize_script(
			'foodforlife', 'foodforlifeData', $foodforlife_data
		);
	...
	public function enqueue_overlay_styles() {
		$debug = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
		wp_enqueue_style('foodforlife-overlay', get_template_directory_uri() . '/assets/css/overlay' . $debug . '.css', array(), \FoodForLife\Helper::get_theme_version());
	}
	...
	public static function change_cart_svg_icon( $cart_icon ) {
		if ( \FoodForLife\Helper::get_option( 'cart_icon_source' ) == 'icon' ) {
			switch( \FoodForLife\Helper::get_option( 'cart_icon' ) ) {
				case 'shopping-bag-2' :
