...
class Helper {
	...
	public static function get_product_countdown( $sale = '', $text = '', $classes = '', $product = null, $expire_date = null ) {
		...
		if ( empty( $sale ) ) {
			$sale = array(
				'weeks'   => esc_html__( 'w', 'foodforlife' ),
		...
		$classes = empty( $classes ) ? 'ffl-product-countdown__countdown' : $classes;
		...
		return sprintf( '<div class="ffl-product-countdown %s">
							%s
							<div class="foodforlife-countdown d-flex justify-content-center lh-1" data-expire="%s" data-text="%s">
	...
	public static function get_date_on_sale_to( $product, $args = array() ) {
		...
		$sale_date = apply_filters( 'foodforlife_product_sale_dates_to', $sale_date );
	...
	public static function get_product_taxonomy( $taxonomy = 'product_cat', $product = false ) {
		...
		if( !is_wp_error( $terms ) && !empty($terms) ) {
			...
			if ( class_exists( 'RankMath' ) ) {
				$primary_term_id = get_post_meta( $product->get_id(), 'rank_math_primary_' . $taxonomy, true );
			}

			if ( ! $primary_term_id && defined( 'WPSEO_VERSION' ) ) {
				$primary_term_id = get_post_meta( $product->get_id(), '_yoast_wpseo_primary_' . $taxonomy, true );
			}
			...
			if ( $primary_term_id ) {
				...
				if ( $primary_term && ! is_wp_error( $primary_term ) ) {
					$terms = array_filter( $terms, function( $term ) use ( $primary_term_id ) {
	...
	public static function lazy_load_image_effect_html() {
		?>
			<div class="ffl-lazy-load-image"></div>
		<?php
	}
	...
	public static function add_to_cart_button($product) {
		$classes = 'ffl-add-to-cart-button button ffl-button-subtle product-loop-button mt-15';
		...
		if( $product->get_type() == 'variable' && \FoodForLife\Helper::get_option( 'product_card_quickadd' ) ) {
			$data_toggle = 'data-toggle="modal"';
			$data_target = 'data-target="quick-view-modal"';
			$classes .= ' foodforlife-quickview-button';
		...
		echo sprintf(
			...
			$product->add_to_cart_text() . esc_attr__( ' for ', 'foodforlife' ) . $product->get_title(),
			$data_toggle,
			$data_target,
			$product->add_to_cart_text()
		);
