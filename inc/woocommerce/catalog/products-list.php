...
class Products_List {
	...
	public function __construct() {
		add_filter( 'foodforlife_product_card_loop_style', array( $this, 'loop_start' ), 20 );
		...
		add_action( 'foodforlife_woocommerce_product_archive_before_content', array( $this, 'add_product_card_list' ) );
		add_action( 'foodforlife_woocommerce_product_archive_after_content', array( $this, 'remove_product_card_list' ) );
	}
	...
	public function loop_start($style){
		...
		if( !empty( $desktop_line_limit ) && $desktop_line_limit !== '3' ) {
			$style .= '--ffl-product-short-description: ' . $desktop_line_limit . ';';
		}
		if( !empty( $tablet_line_limit ) && $tablet_line_limit !== '2' ) {
			$style .= '--ffl-product-short-description-tablet: ' . $tablet_line_limit . ';';
		}
		if( !empty( $mobile_line_limit ) && $mobile_line_limit !== '3' ) {
			$style .= '--ffl-product-short-description-mobile: ' . $mobile_line_limit . ';';
		}
		return $style;
	}
	...
	public function add_product_card_list() {
		...
		if( ! Helper::get_option( 'mobile_product_card_atc' )) {		
			add_action( 'woocommerce_after_shop_loop_item', array( \FoodForLife\WooCommerce\Product_Card\Base::instance(), 'add_to_cart_button_base' ), 30 );
		}
	...
	public function remove_product_card_list() {
		...
		if( ! Helper::get_option( 'mobile_product_card_atc' )) {		
			remove_action( 'woocommerce_after_shop_loop_item', array( \FoodForLife\WooCommerce\Product_Card\Base::instance(), 'add_to_cart_button_base' ), 30 );
		}
		if(class_exists( '\WCBoost\Wishlist\Frontend' ) && Helper::get_option('product_card_wishlist')) {
	...
	public function wishlist_button_lopp_class( $classes ) {
		$classes = 'ffl-button-outline ffl-button-hover-dark';
		return $classes;
	}
