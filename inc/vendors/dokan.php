...
class Dokan {
    ...
    function __construct() {
		...
		add_action( 'foodforlife_woocommerce_product_quickview_summary', array( $this, 'vendor_name_single' ), 20 );
		add_action( 'foodforlife_woocommerce_product_quickadd_summary_header', array( $this, 'vendor_name_single' ), 30 );

		// Catalog sidebar panel
		add_action( 'foodforlife_after_close_site_footer', array( $this, 'button_sidebar' ), 100 );
		add_action( 'foodforlife_after_close_site_footer', array( $this, 'sidebar_panel' ), 100 );
	...
	public function body_classes( $classes ) {
		...
		if ( class_exists( 'Dokan_Pro' ) ) {
			$classes[] = 'foodforlife-dokan-pro';
		}

		if ( function_exists( 'dokan_is_store_listing' ) && dokan_is_store_listing() ) {
			$classes[] = 'foodforlife-dokan-store-list-page';
		}

		if ( function_exists( 'dokan_is_store_page' ) && dokan_is_store_page() ) {
			$classes[] = 'foodforlife-dokan-store-page';
		}
    ...
    public function enqueue_scripts() {
		wp_enqueue_style( 'foodforlife-dokan', get_template_directory_uri() . '/assets/css/vendors/dokan.css', array(), '20241003' );

		wp_enqueue_script( 'foodforlife-dokan', get_template_directory_uri() . '/assets/js/vendors/dokan.js', array(), '20240508', array('strategy' => 'defer') );
	...
	public function vendor_name_loop() {
		...
		$vendor_position = \FoodForLife\Helper::get_option( 'product_card_vendor_position');
	...
	public function vendor_name_single() {
		$vendor_type = \FoodForLife\Helper::get_option( 'single_product_vendor_name' );
	...
	public function vendor_name( $vendor_type, $vendor_position = '' ) {
		...
		$vendor_avatar = $vendor_type == 'avatar' ? get_avatar( $author_id ) : '<span class="vendor-name-text me-3">' . esc_html__( 'By', 'foodforlife' ) . '</span>';
		?>
