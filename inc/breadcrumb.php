	...
	public function get_breadcrumb( $args = '' ) {
		$args = wp_parse_args(
			$args, array(
				...
				'show_on_front'     => true,
				'labels'            => array(
					'home'      => esc_html__( 'Home', 'foodforlife' ),
					'archive'   => esc_html__( 'Archives', 'foodforlife' ),
					'blog'      => esc_html__( 'Blog', 'foodforlife' ),
					'search'    => esc_html__( 'Search results for', 'foodforlife' ),
					'not_found' => esc_html__( 'Not Found', 'foodforlife' ),
					'portfolio' => esc_html__( 'Portfolio', 'foodforlife' ),
					'author'    => esc_html__( 'Author:', 'foodforlife' ),
					'day'       => esc_html__( 'Daily:', 'foodforlife' ),
					'month'     => esc_html__( 'Monthly:', 'foodforlife' ),
					'year'      => esc_html__( 'Yearly:', 'foodforlife' ),
				),
			)
		);

		$args = apply_filters( 'foodforlife_breadcrumbs_args', $args );
		...
		if ( ! $args['home_class'] ) {
			...
			$items[] = sprintf(
				...
				$args['before_item'],
				$args['home_class'],
				apply_filters( 'foodforlife_breadcrumbs_home_url', get_home_url() ),
				$args['labels']['home'],
				$args['after_item']
			);
		...
		elseif ( is_single() ) {
			...
			$parent_post = apply_filters('foodforlife_breadcrumbs_get_parent_post', '', $item_tpl);
		...
		elseif ( is_search() ) {
			...
			$search_item = apply_filters( 'foodforlife_breadcrumbs_search_item', $search_item );
		...
		elseif ( is_day() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'foodforlife' ), $args['labels']['day'], get_the_date() )
			);
		} // Month archive
		elseif ( is_month() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'foodforlife' ), $args['labels']['month'], get_the_date( 'F Y' ) )
			);
		} // Year archive
		elseif ( is_year() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'foodforlife' ), $args['labels']['year'], get_the_date( 'Y' ) )
			);
		} // Archive
		else {
			$items[] = sprintf(
		...
		$items = apply_filters('foodforlife_get_site_breadcrumb',  $items );
